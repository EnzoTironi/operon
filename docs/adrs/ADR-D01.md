# ADR-D01 - Wastewater permit-margin equality
...
The declarative rule accepts PredCOD <= 0.9 * Limit; the minimal verdict hook denies only values above 0.9 * Limit. The boundary-test table instead denies exactly 0.9 * Limit. These produce different decisions at equality.

## Decision / required resolution

A domain/source owner selects and versions the comparator, updates the normative rule and table together, and approves tests just below, at and above the threshold. Keep the discrepancy visible until then; this plan chooses neither clinical nor industrial operating limits.
