# ADR-D09 - Impact narrative examples lack detailed business contracts
...
Aviation and SOMPO narrative examples identify problems and vendor outcomes, not complete input schemas, rule sets, permissions, error behavior or acceptance datasets for Operon.

## Decision / required resolution

For each advertised optional domain package, obtain an explicit owner-approved workflow contract and test corpus. Otherwise retain these passages as context; do not invent rules or borrow the vendor impact statistics as Operon outcomes.
