...
Qualify Node/PostgreSQL as the reference operational deployment; preserve separate local SQLite and Cloudflare profiles.

## Context and trade-off

A single demonstrated local transaction boundary simplifies the initial durable contract. It does not solve external atomicity.
...
Shared adapter conformance, crash recovery, concurrency and declared provider limitations.
