# ADR-D08 - Domain completeness and time-budget parameters
...
Examples leave configuration choices such as N readings, sufficient sensor coverage, permitted gaps, source qualification, local-day timezone and exact missingness rules to the decision contract. A numeric age threshold alone is not a complete temporal/evidence specification.

## Decision / required resolution

Have the domain owner supply R(d), units, sampling/coverage predicates, timezone, clock tolerance, freshness inequalities and exemption policy before executing boundary tests. Keep teaching fixtures separate from operational policies.
