# ADR-D15 - Promotion unit and plan semantics
...
**Status:** APPROVED_FOR_V0  
**Applicability:** program contract

## Context

Decide which outcomes map to one atomic business action versus an explicit multi-step workflow.

## Decision / required resolution

V0 supports bounded registered actions. Larger multi-step plans and scenario promotion remain post-V0 unless separately reviewed. Local atomicity and remote reconciliation stay explicit; scenario state is never copied into production truth.
