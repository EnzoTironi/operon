# ADR-D06 - Shadow-cutover policy for intentional changes
...
The healthcare test says to prohibit cutover while the dual-engine difference report is non-empty. General change governance also allows reviewed rule changes that can intentionally alter outcomes. Whether the report means all differences or unresolved differences must be decided explicitly.

## Decision / required resolution

Define the release-specific policy. Under the literal no-differences rule, block until empty. A policy permitting approved intentional differences requires an explicit authorized amendment, not a silent weakening by an implementer.
