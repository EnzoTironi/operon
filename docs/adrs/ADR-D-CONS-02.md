# ADR-D-CONS-02 - Reference deployment
...
Retain Effect. Qualify Node/PostgreSQL as reference; separately certify SQLite and Cloudflare profiles. No framework rewrite.

## Decision / required resolution

V0 local durable profile is SQLite with shared persistent state for development/benchmark only. V0 real-company reference profile is Node/PostgreSQL. Cloudflare, sovereign and other profiles require separate qualification; no profile pass implies another.
