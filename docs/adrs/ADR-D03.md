# ADR-D03 - Wastewater applicability envelope versus partial hook
...
The model declaration uses influent COD < 800 and water temperature > 8 degrees C. The minimal hook only reviews COD > 800 and omits the temperature check. Exact COD equality and temperature handling are therefore not established by that hook.

## Decision / required resolution

Confirm that the full applicability declaration is normative, explicitly bind its inputs, and test each boundary and missing input. Do not represent the shortened example as the complete scope check.
