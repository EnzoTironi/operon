...
Local atomic commit contains state, decision, approval, deduplication, reservation and outbox.

## Context and trade-off

Blind rollback is not concurrency-safe and a JSON audit sidecar is not the transaction boundary.
...
Actual database faults, kill/restart, delayed owners and unknown remote outcomes.
