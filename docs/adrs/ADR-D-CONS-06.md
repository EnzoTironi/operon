# ADR-D-CONS-06 - Initial execution order
...
Begin with bounded CLI/verifier fixes and durable/authentication contracts while Studio/sentinel/scenario work runs in parallel against explicit interfaces. Maximum scope is retained.

## Decision / required resolution

Execute V0-A protocol/state, V0-B public authoring, V0-C ingestion/provenance, V0-D reconciliation/mirror, V0-E safe use/action/views, then mandatory F1 and F2. Post-V0 work may run off-path only against accepted contracts.
