...
Retain TypeScript and Effect; reproduce the exact candidate before changes.

## Context and trade-off

The runtime already uses these abstractions. A framework rewrite would obscure fixes without supplying guarantees.
...
Actual dependency install, typecheck, full tests and Effect interruption/laziness/resource lifecycle.
