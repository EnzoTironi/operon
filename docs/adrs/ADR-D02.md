# ADR-D02 - Clinical absolute-ceiling equality
...
The teaching code denies D > Max, which does not deny equality. The boundary-test table says a value exactly at the absolute ceiling must be denied. The chapter explicitly says its numbers are pedagogical, not clinical operating guidance.

## Decision / required resolution

Resolve the illustrative comparator in a versioned rule decision. Real clinical limits require qualified clinical ownership and separate validation. Do not mark both conflicting expected outcomes as passing the same rule version.
