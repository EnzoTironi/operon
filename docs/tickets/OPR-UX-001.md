# OPR-UX-001 - Evidence-first workbench
...
Provide model/schema inspection, object/search views, freshness/provenance, inbox, decision history and governance views.
...
- **UX-001.T01:** Navigate the actual served app, inspect evidence, propose, switch to an independent reviewer, approve and execute. **Expected:** User actions cause intended real state transitions; no fabricated API responses.
- **UX-001.T02:** Show missing/stale/conflicting data and server denial/unavailability. **Expected:** Warnings and reasons are conspicuous; unavailable is not rendered as empty-but-valid state.
...
Requirement IDs: UX-001. Acceptance case IDs: UX-001.T01, UX-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
