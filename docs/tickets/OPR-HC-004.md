# OPR-HC-004 - Clinical authority and information-only automation
...
Use the chapter’s teaching role matrix and qualification checks; automatic alarms are distinct from physical/dosage actions.
...
- **HC-004.T01:** Attempt review with ordinary nurse, expired specialist credential, physician and countersigner identities. **Expected:** Only declared current authority can confirm the relevant risk band; required countersignatures apply.
- **HC-004.T02:** Trigger an urgent synthetic alarm and attempt automatic treatment adjustment. **Expected:** Alarm/notification can escalate to people; the runtime cannot silently automate dosage/physical action.
...
Requirement IDs: HC-004. Acceptance case IDs: HC-004.T01, HC-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
