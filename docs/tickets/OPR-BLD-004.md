# OPR-BLD-004 - Distributable package validation
...
Test installation and execution from the built release artifact, outside the development workspace.
...
- **BLD-004.T01:** Pack each public package; install them into a clean consumer project and import declared entrypoints. **Expected:** Exports, declarations, assets and migrations resolve without imports from source-only paths.
- **BLD-004.T02:** Follow the documented start command with an empty database; stop and restart. **Expected:** A usable local instance starts, credentials are explicitly provisioned, and non-demo state persists.
...
Requirement IDs: BLD-004. Acceptance case IDs: BLD-004.T01, BLD-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
