# OPR-SCN-006 - Production secrets and credentials are absent
...
Scenario workloads receive no production database passwords, connector tokens, cloud metadata credentials, signing keys or authority-bearing audit/grant material.
...
- **SCN-006.T01:** Inspect the actual workload mounts, process environment, images, build caches and allowed identity endpoints from inside the runtime. **Expected:** Only scoped scenario credentials exist; no host or production secret is reachable.
- **SCN-006.T02:** Seed an innocuous canary in protected configuration and attempt access through subprocesses, logs, SSRF or inherited environment. **Expected:** The canary is not disclosed and the blocked attempt is independently recorded.
...
Requirement IDs: SCN-006. Acceptance case IDs: SCN-006.T01, SCN-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
