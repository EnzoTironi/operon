# OPR-BLD-003 - Strict build and type-negative tests
...
Build every package and generated client against actual dependency declarations.
...
- **BLD-003.T01:** Clean all dist files and build packages and examples in dependency order. **Expected:** No unresolved types, stale artifacts or hidden workspace aliases are required.
- **BLD-003.T02:** Compile clients with wrong action parameters, missing service requirements and invalid enum values. **Expected:** The intended invalid programs fail typechecking; valid equivalents compile. Unnecessary any must not erase these checks.
...
Requirement IDs: BLD-003. Acceptance case IDs: BLD-003.T01, BLD-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
