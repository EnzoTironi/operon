# OPR-IBAC-029 - Portable accessible purpose packages
...
Export versioned purposes, mandates, grant schemas and decision records with precise semantics; local deterministic operation is possible without a paid model.
...
- **IBAC-029-A:** Install a domain purpose package locally, execute the reference workflow and export/import supported policy semantics. **Expected:** The workflow works with authenticated structured task input and preserves supported semantics and evidence.
- **IBAC-029-B:** Import an unsupported policy constraint or run with an unavailable optional model provider. **Expected:** Unsupported meaning is rejected; the deterministic authorized path remains available or a truthful dependency block is shown.
...
Requirement IDs: IBAC-029. Acceptance case IDs: IBAC-029-A, IBAC-029-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
