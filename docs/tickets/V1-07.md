# V1-07 - Promotion, rollout and rollback ledger
...
- `packages/release/src/promotion/`
- `packages/release/src/rollout/`
- `packages/release/src/rollback/`
- `tests/release/`

## Environment

- `OPERON_RELEASE_CHANNEL`
- `OPERON_ROLLOUT_STEP`
- `OPERON_EVIDENCE_DIR`
...
```ts
promote(plan, evidenceSet): Promise<PromotionReceipt>;
rollback(receipt, reason): Promise<RollbackReceipt>;
```
...
- [x] promotion rejects evidence from another candidate/profile.
- [x] failed rollout stops before next cohort.
- [x] rollback preserves history and reports uncompensated effects.
