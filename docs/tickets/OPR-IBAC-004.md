# OPR-IBAC-004 - Non-expanding grants
...
At a fixed policy version a grant can only narrow base, delegator and agent authority. Grant issuance is an explicit trusted operation.
...
- **IBAC-004-A:** Generate bounded grants from several narrower tasks and compare effective action/resource sets to each issuer envelope. **Expected:** Every grant is a subset of the allowed envelope; lower-trust added text cannot increase the set.
- **IBAC-004-B:** Request an out-of-envelope action with a convincing rationale or client-supplied low impact. **Expected:** Issuance is denied or narrowed; no extra authority is minted.
...
Requirement IDs: IBAC-004. Acceptance case IDs: IBAC-004-A, IBAC-004-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
