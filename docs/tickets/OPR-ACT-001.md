# OPR-ACT-001 - Complete action contract
...
Version typed parameters, criteria, permissions, ontology edits, function backing and declared external effects.
...
- **ACT-001.T01:** Register a mutating action without a mutation handler or effect declaration. **Expected:** Reject incomplete registration; an intentional no-op/notification action is explicitly classified.
- **ACT-001.T02:** Submit a registered mutation via HTTP, MCP and SDK, then read state independently. **Expected:** Intended fields, links and revisions change through the common governed path.
