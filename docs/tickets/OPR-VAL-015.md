# OPR-VAL-015 - Real-dependency execution and blocked status
...
The product gate runs the pinned build/types/test suite and concrete adapters; unavailable dependencies and skipped applicable tests remain blocked, not passes.
...
- **VAL-015.T01:** Remove registry/cache or stop the selected database **Expected:** Report BLOCKED_ENVIRONMENT with exact cause, separate from product failure
- **VAL-015.T02:** Run the fully installed toolchain with a deliberate type/test failure **Expected:** The gate fails on actual failure and records real library versions
...
Requirement IDs: VAL-015. Acceptance case IDs: VAL-015.T01, VAL-015.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
