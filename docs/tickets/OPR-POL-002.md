# OPR-POL-002 - Role, classification and purpose policies
...
Compose role-based, marking/classification-based and purpose-based policy; purpose is recorded and governed, not self-authorization.
...
- **POL-002.T01:** Use a treatment-scoped credential to request research-only data, or vice versa. **Expected:** Denied unless an explicit reviewed policy grants the combination; purpose is audited.
- **POL-002.T02:** Raise an input’s sensitivity marking while a session/cache exists. **Expected:** Derived visibility and cached outputs are rechecked; classification cannot silently drop.
...
Requirement IDs: POL-002. Acceptance case IDs: POL-002.T01, POL-002.T02, BAI-043, BAI-044, BAI-045, BAI-046, BAI-047, BAI-048. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
