...
Exportação e importação devem preservar identidades e histórico autorizado sem reemitir efeitos.
...
- proposed packages/federation/

## Steps

1. Resolve and freeze the relevant S15 contract; retain existing source vocabulary and open decisions.
...
- **FULL-ACC-045:** Given um tenant é restaurado em célula limpa; when a verificação de migração roda **Expected:** consultas e dossiês permitidos coincidem e nenhuma notificação histórica é reenviada
...
Requirement IDs: FULL-045. Acceptance case IDs: FULL-ACC-045. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
