# OPR-QRY-001 - Four Object Set forms
...
Implement static, dynamic, temporary and persistent sets with their distinct membership and lifecycle semantics.
...
- **QRY-001.T01:** Update an object so it enters/leaves a dynamic set; compare with a static set. **Expected:** Dynamic membership reevaluates at query time; static membership changes only explicitly.
- **QRY-001.T02:** Close a session containing a temporary set and reopen a named persistent set. **Expected:** Temporary scope expires; saved definitions retain identity and version.
...
Requirement IDs: QRY-001. Acceptance case IDs: QRY-001.T01, QRY-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
