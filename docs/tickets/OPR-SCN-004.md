# OPR-SCN-004 - No cross-scenario or production state leakage
...
Objects, links, search indexes, embeddings, derived caches, event streams and artifacts are scoped to environment and baseline with a read-only authorized base and writable private overlay.
...
- **SCN-004.T01:** Create identical IDs in two scenarios and production, mutate/delete/link one, then independently query every scope and cache. **Expected:** Only the targeted overlay changes and copied relationships do not reference writable production state.
- **SCN-004.T02:** Omit scope from cache keys, traversal jobs or event consumers in a deliberate mutation test. **Expected:** The acceptance suite detects leakage; release is blocked until every advertised path is isolated.
...
Requirement IDs: SCN-004. Acceptance case IDs: SCN-004.T01, SCN-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
