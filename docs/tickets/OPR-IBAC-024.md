# OPR-IBAC-024 - Transport and scheduled-job parity
...
HTTP, actual MCP clients, SDK, CLI and scheduled jobs use the same authorization and execution boundary.
...
- **IBAC-024-A:** Submit the same permitted mutation through each supported interface and compare decisions and independent state. **Expected:** Equivalent trusted requests have equivalent policy results and effects.
- **IBAC-024-B:** Use a transport-specific raw store, optional security engine, synthetic human subject or missing purpose/grant field. **Expected:** The bypass is rejected; unsupported interfaces are explicitly disabled instead of silently less protected.
...
Requirement IDs: IBAC-024. Acceptance case IDs: IBAC-024-A, IBAC-024-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
