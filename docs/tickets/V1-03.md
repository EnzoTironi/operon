# V1-03 - Principal, grant and authority evaluator
...
- `packages/kernel/src/auth/`
- `packages/kernel/src/authority/`
- `packages/adapters/src/auth/`
- `tests/authority/`
...
```ts
evaluate(input:{principal:PrincipalContext; intent:IntentGrant; uses:UseGrant[]}): AuthorityResult;
```
...
- [x] client-supplied tenant/role cannot elevate authority.
- [x] aggregate-use budget exhaustion denies or requires review.
- [x] result is exactly ALLOW, DENY, REVIEW_REQUIRED or EVIDENCE_INSUFFICIENT; infra failure is separate.
