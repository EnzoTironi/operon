# OPR-OPS-001 - Durable live configuration
...
Every claimed deployment wires real storage, queues, secrets and authentication; a resource manifest or empty worker is not a deployment test.
...
- **OPS-001.T01:** Deploy with actual bindings, create/approve a case, restart the process/container/worker and read it. **Expected:** Business state, inbox and pending delivery survive.
- **OPS-001.T02:** Remove a required binding in the actual deployment target. **Expected:** Startup/health indicates non-readiness rather than falling back to in-memory success.
