# OPR-IBAC-025 - Mandate-fidelity evaluation
...
Evaluate natural-language mandate generation separately from policy enforcement and execution; structured task entry remains supported.
...
- **IBAC-025-A:** Use independently labeled authorized tasks with paraphrases and real selected models plus deterministic forms. **Expected:** Report faithful mandate rate, legitimate completion and review burden for each input path.
- **IBAC-025-B:** Use ambiguous tasks, adversarial paraphrases and seemingly helpful scope expansion on a held-out corpus. **Expected:** Unsafe accepted authority is measured separately from completed unsafe actions; ambiguity is narrowed or reviewed rather than hidden.
...
Requirement IDs: IBAC-025. Acceptance case IDs: IBAC-025-A, IBAC-025-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
