# OPR-OPS-002 - Cloud portability claims
...
Validate each supported target’s actual APIs, resource lifecycle and Effect platform adapters independently.
...
- **OPS-002.T01:** Build and run the shipped worker on each advertised target, including migrations and auth. **Expected:** No unsupported Node/crypto/SQLite assumptions or fictitious cloud bindings.
- **OPS-002.T02:** Provision, update and tear down the declared queue/database/object-store resources in a disposable project. **Expected:** Actual resources behave as configured; destructive actions require explicit controls.
...
Requirement IDs: OPS-002. Acceptance case IDs: OPS-002.T01, OPS-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
