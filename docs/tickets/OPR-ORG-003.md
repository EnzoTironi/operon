# OPR-ORG-003 - Measured decision value and failure modes
...
**Depends on:** WS01, WS03, WS04, WS06
...
Derive operational indicators from recorded events, including overrides, confidence/readiness, friction and unused-action review.
...
- packages/osdk/src/
- proposed apps/studio/
- proposed apps/workspace/

## Steps

1. Resolve and freeze the relevant S13 contract; retain existing source vocabulary and open decisions.
...
- **ORG-003.T01:** Calculate acceptance/override, cycle time, freshness, reconciliation and per-decision cost on a hand-counted fixture. **Expected:** Counts and denominators agree; declined/unready cases are not hidden to improve apparent performance.
- **ORG-003.T02:** Review ontology inflation, indigestion, unused actions, deferred governance, new silos, LLM overreach, vanity metrics, rollback overclaim, lost versions and trust erosion. **Expected:** Each has owner, signal and response; organizational findings are not misreported as unit-test passes.
...
Requirement IDs: ORG-003. Acceptance case IDs: ORG-003.T01, ORG-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
