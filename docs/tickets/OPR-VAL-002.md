# OPR-VAL-002 - Strict CLI parsing and result envelope
...
Reject unknown flags, missing values, invalid enumerations and non-finite numbers; emit a versioned single JSON envelope and documented exit status on all machine-mode paths.
...
- .cursor/skills/verify-operon/

## Steps

1. Resolve and freeze the relevant S18 contract; retain existing source vocabulary and open decisions.
...
- **VAL-002.T01:** Pass --input where only --inputs exists, and an out-of-range tier **Expected:** Command fails with structured usage error rather than using a default
- **VAL-002.T02:** Execute valid forms with spaces/Unicode/quotes and expected failures **Expected:** No shell interpretation; exactly one valid envelope and appropriate exit
...
Requirement IDs: VAL-002. Acceptance case IDs: VAL-002.T01, VAL-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
