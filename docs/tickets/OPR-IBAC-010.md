# OPR-IBAC-010 - Claim consistency is not causal proof
...
Separate supported observations, relevance, policy eligibility and expected outcomes; semantic uncertainty cannot add authority.
...
- **IBAC-010-A:** Supply true relevant evidence for a policy-permitted action and record each separate adjudication stage. **Expected:** The receipt distinguishes factual support from authorization and predicted business benefit.
- **IBAC-010-B:** Supply a true observation but propose an unrelated or prohibited response, or a contradictory claim. **Expected:** The response is denied/reviewed under policy; contradiction is not automatically labeled fraud.
...
Requirement IDs: IBAC-010. Acceptance case IDs: IBAC-010-A, IBAC-010-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
