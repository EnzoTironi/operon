# OPR-SEN-010 - Bounded review loops and legitimate appeal
...
Revision, resubmission and appeal preserve prior findings and enforce limits on model calls, parallel reviewers and elapsed time; a positive retry does not erase prior unresolved concerns.
...
- D11 [OPEN]: Review and approve an explicit versioned contract; the proposed default is not recorded as approved.
...
- **SEN-010.T01:** Submit corrected evidence after a hold and have an authorized human resolve a documented disagreement. **Expected:** A linked revision/appeal records the resolution without fabricating human identity or waiving non-waivable policy.
- **SEN-010.T02:** Keep rephrasing and spawning reviewers until one recommends proceed, or exhaust review compute. **Expected:** Retry/budget policy stops shopping and routes unresolved work explicitly; exhaustion does not authorize execution.
...
Requirement IDs: SEN-010. Acceptance case IDs: SEN-010.T01, SEN-010.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
