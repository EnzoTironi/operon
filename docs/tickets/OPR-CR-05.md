# OPR-CR-05 - Repair: Approval persistence is scoped to an in-process object reference
...
Persist proposals, reviewed snapshots, claims, expiry, reviewers and operation linkage in the durable cell; inject scoped services instead of global sharing.
...
- packages/runtime/src/inbox.ts:35-59
...
- **CR-05.REGRESSION:** Create in MCP, restart both service and process, then approve through an independently authenticated channel; recover exactly one operation. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
