# OPR-VAL-017 - Corruption quarantine
...
Malformed or invalid persisted control state must not be silently discarded or rewritten as a healthy empty state.
...
- **VAL-017.T01:** Corrupt/truncate the CLI sidecar or durable control records then run doctor **Expected:** Explicit recovery-required failure, no silent overwrite
- **VAL-017.T02:** Recover a valid backup under authorized procedure **Expected:** Verified history restores with observed limitations
...
Requirement IDs: VAL-017. Acceptance case IDs: VAL-017.T01, VAL-017.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
