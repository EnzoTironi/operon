# OPR-EXT-003 - Retry and remote idempotency
...
Use stable per-command keys with tested receiver behavior and bounded retry/dead-letter policy.
...
- **EXT-003.T01:** A real test receiver durably accepts and drops the response; retry with the same key. **Expected:** A receiver with declared deduplication produces one effect; otherwise the command is unresolved, not called exactly-once.
- **EXT-003.T02:** Return transient failures beyond the retry ceiling and a permanent rejection. **Expected:** Correct attempt history and dead-letter disposition; permanent errors do not loop forever.
...
Requirement IDs: EXT-003. Acceptance case IDs: EXT-003.T01, EXT-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
