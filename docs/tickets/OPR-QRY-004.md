# OPR-QRY-004 - No projection side channels
...
Apply caller policy consistently before observations through sets, counts, sorting, aggregations, readiness and exports.
...
- packages/runtime/src/bitemporal-store.ts
- packages/runtime/src/oss.ts

## Steps

1. Resolve and freeze the relevant S04 contract; retain existing source vocabulary and open decisions.
...
- **QRY-004.T01:** Use two principals on get/list/set/traverse/aggregate/readiness/inbox/audit/export and direct SDK OSS access. **Expected:** No restricted row, field or inferred aggregate leaks through an alternate route.
- **QRY-004.T02:** Filter, sort or group by a forbidden field while omitting it from the projection. **Expected:** The operation is denied or explicitly declassified; omission from the output alone is insufficient.
...
Requirement IDs: QRY-004. Acceptance case IDs: QRY-004.T01, QRY-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
