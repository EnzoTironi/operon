# OPR-IBAC-026 - Effect scope and interruption behavior
...
Real Effect layers isolate trusted context, release resources and preserve the transactional boundary under cancellation.
...
- **IBAC-026-A:** Run concurrent tenants and task scopes through actual live services and test success, typed failure and defect cleanup. **Expected:** No context/credential leakage, resource leak or eager approval claim occurs.
- **IBAC-026-B:** Interrupt before commit and after commit-before-response; restart the runtime and retry. **Expected:** Precommit work is absent, committed receipts recover exactly once, and no orphan worker gains broader scope.
...
Requirement IDs: IBAC-026. Acceptance case IDs: IBAC-026-A, IBAC-026-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
