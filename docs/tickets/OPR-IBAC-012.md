# OPR-IBAC-012 - Actual-effect containment
...
The runtime validates prepared effects against grant bounds and commits only that checked semantic package.
...
- **IBAC-012-A:** Execute a registered action with checked objects, links, fields, recipients and handler version. **Expected:** Independent state inspection matches the authorized footprint exactly.
- **IBAC-012-B:** Modify the handler, add an extra target, or swap a recipient after validation or approval. **Expected:** The digest/version check invalidates the operation or requires reevaluation; extra effects cannot commit.
...
Requirement IDs: IBAC-012. Acceptance case IDs: IBAC-012-A, IBAC-012-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
