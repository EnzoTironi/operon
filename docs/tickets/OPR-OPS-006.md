# OPR-OPS-006 - Worker observability and incident handling
...
Monitor freshness, queue lag, approval aging, reconciliation, failure causes and decision trace IDs; alert on required-control outages.
...
- **OPS-006.T01:** Disable auth/verifier/audit/delivery or exhaust storage and watch operational signals. **Expected:** Readiness, alerts and user-visible statuses reflect the actual impairment; no healthy-but-unsafe signal.
- **OPS-006.T02:** Trace a failed decision through API, policy, transaction and connector records. **Expected:** An operator can identify cause, affected scope and recovery action without leaked secrets.
...
Requirement IDs: OPS-006. Acceptance case IDs: OPS-006.T01, OPS-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
