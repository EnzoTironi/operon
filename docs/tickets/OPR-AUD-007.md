# OPR-AUD-007 - Audit permissions and presentation evidence
...
Protect dossiers and source spans like the underlying evidence; capture what the interface presented without claiming to prove attention.
...
- **AUD-007.T01:** A principal permitted to see a decision requests restricted evidence or actor details through its dossier/export. **Expected:** Field/source policy still applies; audit is not a bypass channel.
- **AUD-007.T02:** Render a proposal, then change displayed evidence before later reconstruction. **Expected:** Receipt identifies the displayed version and time; it does not assert the human read or understood it.
...
Requirement IDs: AUD-007. Acceptance case IDs: AUD-007.T01, AUD-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
