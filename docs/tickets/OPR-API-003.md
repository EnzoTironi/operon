# OPR-API-003 - Real MCP protocol compatibility
...
Pin the supported MCP version/transports and test initialize, discovery, calls, schemas, errors and shutdown with a real client.
...
- **API-003.T01:** Use the actual MCP SDK/client to initialize, list tools and call read/propose tools over each claimed transport. **Expected:** Protocol negotiation, schemas and outputs conform; a JSON-RPC lookalike is not called full MCP.
- **API-003.T02:** Test unsupported versions, unauthorized tools, request cancellation and session-bound credentials. **Expected:** No session confusion or authority leak; stdio and HTTP use their correctly declared authentication models.
...
Requirement IDs: API-003. Acceptance case IDs: API-003.T01, API-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
