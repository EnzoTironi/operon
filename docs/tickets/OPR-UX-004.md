# OPR-UX-004 - Browser boundary validation
...
Validate real browser-server navigation, authentication storage, origin policy and content-security behavior, not DOM relay alone.
...
- **UX-004.T01:** Run the real application in a browser over its deployed origin with actual network/auth requests. **Expected:** Login/session/approval work and security policies enforce as configured.
- **UX-004.T02:** Render hostile evidence text, switch users and expire a session. **Expected:** No script execution, stale authority or cached protected evidence survives improperly.
...
Requirement IDs: UX-004. Acceptance case IDs: UX-004.T01, UX-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
