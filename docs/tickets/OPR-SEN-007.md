# OPR-SEN-007 - Sequence-aware intent and cumulative-impact review
...
Review considers the authoritative task history, grants and aggregate reservations, not only the last tool call; cumulative limits are still enforced deterministically.
...
- **SEN-007.T01:** Split a task into individually small operations with a collectively disallowed recipient pattern or budget impact. **Expected:** The review reports the sequence context and the runtime prevents aggregate-limit violations.
- **SEN-007.T02:** Run the split concurrently across agents, sessions and tools with the sentinel forced to recommend proceed. **Expected:** Persistent budgets and task boundaries remain enforced under concurrent commits.
...
Requirement IDs: SEN-007. Acceptance case IDs: SEN-007.T01, SEN-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
