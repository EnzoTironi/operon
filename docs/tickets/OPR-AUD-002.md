# OPR-AUD-002 - Canonical persisted representation
...
Normalize/validate the value model and hash exactly the representation stored, exported and reloaded.
...
- **AUD-002.T01:** Include optional undefined fields, nested objects, Unicode and supported dates/numbers, then serialize/reload. **Expected:** Accepted normalized records retain identical digest; unsupported values fail before append.
- **AUD-002.T02:** Reorder object keys, vary unsupported numeric representations and mutate content. **Expected:** Key order alone does not change the digest; meaningful supported content changes are detected.
...
Requirement IDs: AUD-002. Acceptance case IDs: AUD-002.T01, AUD-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
