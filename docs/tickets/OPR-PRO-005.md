# OPR-PRO-005 - Action implementation and declared effects agree
...
The production candidate is executed only by registered, pinned handlers whose actual prepared effects are checked against the manifest and current grant.
...
- **PRO-005.T01:** Submit a selected action through each transport and independently compare committed writes/commands with the validated effects. **Expected:** Execution is within the approved manifest and the live target changes as intended.
- **PRO-005.T02:** Change the handler to introduce hidden writes or direct network access while preserving its public action name. **Expected:** Version/integrity/enforcement checks block the candidate and no out-of-plan operation is authorized.
...
Requirement IDs: PRO-005. Acceptance case IDs: PRO-005.T01, PRO-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
