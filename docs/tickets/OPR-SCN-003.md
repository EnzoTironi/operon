# OPR-SCN-003 - Pinned validation baselines and explicit rebasing
...
Validation runs pin an immutable baseline plus schema/policy/model/fixture versions; exploratory rolling scenarios are explicitly labeled and each rebase invalidates dependent evidence.
...
- D12 [OPEN]: Review and approve an explicit versioned contract; the proposed default is not recorded as approved.
...
- **SCN-003.T01:** Replay a pinned scenario after main data changes, then explicitly rebase a working scenario. **Expected:** Pinned results retain their inputs; rebase creates a new baseline/digest and prior reports are marked stale.
- **SCN-003.T02:** Change live data behind a lazy overlay or import an auto-rebased scenario as historical proof. **Expected:** The provenance check rejects the claimed fixed baseline or records the new one; historical-snapshot claims are not inferred.
...
Requirement IDs: SCN-003. Acceptance case IDs: SCN-003.T01, SCN-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
