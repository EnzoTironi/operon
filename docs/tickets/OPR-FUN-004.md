# OPR-FUN-004 - Model lifecycle and applicability
...
Register model versions with training/evaluation evidence, scope declarations and governed release.
...
- **FUN-004.T01:** Supply inputs outside the model’s declared applicability envelope. **Expected:** Prediction explicitly reports out_of_scope and cannot independently authorize an action.
- **FUN-004.T02:** Attempt unreviewed replacement of a production model or its scope threshold. **Expected:** Branch/review/release is required; existing predictions keep their original model ID.
...
Requirement IDs: FUN-004. Acceptance case IDs: FUN-004.T01, FUN-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
