# OPR-QRY-002 - Durable permission scopes
...
Permission-bearing sets must be persistent/versioned; changes in dynamic membership affect authorization immediately.
...
- **QRY-002.T01:** Try installing a policy referencing a session-only selection. **Expected:** Policy registration rejects an unauditable scope.
- **QRY-002.T02:** Transfer an object out of a caller’s permitted set during an active session. **Expected:** Subsequent reads/functions/actions and cached results cannot use the old scope.
...
Requirement IDs: QRY-002. Acceptance case IDs: QRY-002.T01, QRY-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
