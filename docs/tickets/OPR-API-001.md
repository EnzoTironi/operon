...
Route HTTP, MCP, SDK and CLI business operations to the same authenticated governed services.
...
- **API-001.T01:** Run identical registered object-changing actions through every supported interface. **Expected:** Equivalent state/dossier outcomes with correct transport/caller metadata; independent reads confirm changes.
- **API-001.T02:** Repeat forbidden/missing-evidence/stale-approval cases through every interface. **Expected:** No weaker alternate route bypasses a guard.
...
Requirement IDs: API-001. Acceptance case IDs: API-001.T01, API-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
