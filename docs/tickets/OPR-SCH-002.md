# OPR-SCH-002 - Explicit missing-value semantics
...
Distinguish absent, unknown, not applicable, null and legitimate zero/false values according to the declared contract.
...
- **SCH-002.T01:** Evaluate required/optional fields with absent, null, unknown, not-applicable, zero, false and empty string inputs. **Expected:** No truthiness coercion changes their declared meaning; missing evidence is not automatically false.
- **SCH-002.T02:** Use invalid numeric ranges, non-finite numbers, malformed timestamps and enum values. **Expected:** Decode fails before actions or stored evidence consume the values.
...
Requirement IDs: SCH-002. Acceptance case IDs: SCH-002.T01, SCH-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
