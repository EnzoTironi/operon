# OPR-PRO-007 - Atomic local promotion with honest multi-step semantics
...
A bounded local action commits edits, dossier, usage reservations and outbound commands atomically. Larger plans expose authorized step boundaries and compensation, not false global atomicity.
...
- D15 [APPROVED_FOR_V0]: V0 supports bounded registered actions. Larger multi-step plans and scenario promotion remain post-V0 unless separately reviewed. Local atomicity and remote reconciliation stay explicit; scenario state is never copied into production truth.
...
- **PRO-007.T01:** Crash a promotion at each persistence boundary and inspect reopened storage; separately execute a declared multi-step plan. **Expected:** The local action is whole or absent; a multi-step plan records exactly which steps committed.
- **PRO-007.T02:** Fail a later step after an irreversible remote effect and request a universal rollback. **Expected:** The system reports the committed outcome and follows an explicit compensation/reconciliation policy.
...
Requirement IDs: PRO-007. Acceptance case IDs: PRO-007.T01, PRO-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
