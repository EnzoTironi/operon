# OPR-UNI-002 - Scenario-to-production reference workflow
...
One real supported workflow demonstrates trusted task, independent scenario alternatives, sentinel review, exact promotion approval, real commitment, receiver outcome and audit reconstruction.
...
- **UNI-002.T01:** Run the same inventory-reallocation case through actual HTTP/MCP/SDK/UI with real Effect, durable storage, a live review model and an independent local receiver. **Expected:** Scenario isolation, production scope, separate human identity, actual mutation and delivery are independently evidenced; local receiver is not misrepresented as an ERP.
- **UNI-002.T02:** Repeat with injected stale evidence, denied purpose, forged review, altered recipient and remote timeout. **Expected:** Each adverse case has the correct non-success state with no unauthorized operation.
...
Requirement IDs: UNI-002. Acceptance case IDs: UNI-002.T01, UNI-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
