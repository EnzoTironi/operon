# OPR-OPS-008 - Distributed correctness only when claimed
...
Use shared coordination and tested failover when supporting multiple processes/nodes; process-local maps are not distributed locks.
...
- D10 [OPEN]: Record workload, environment, SLOs, RPO/RTO, retention and intended target deployments before performance/failover acceptance. Report measurements and limits; no universal everyone-scale or zero-downtime assertion.
...
1. Resolve and freeze the relevant S16 contract; retain existing source vocabulary and open decisions.
...
- **OPS-008.T01:** Run separate processes/nodes against the claimed shared backend and race action/delivery claims. **Expected:** Global invariants hold without shared in-process memory.
- **OPS-008.T02:** Partition workers or fail the active coordinator/database according to supported topology. **Expected:** Authority, durability and recovery match explicit assumptions; unsupported HA is disclosed.
...
Requirement IDs: OPS-008. Acceptance case IDs: OPS-008.T01, OPS-008.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
