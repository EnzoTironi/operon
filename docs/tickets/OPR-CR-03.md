# OPR-CR-03 - Repair: Static maps are not distributed coordination
...
Use the selected durable cell/database or qualified coordinator; bind resource names to tenant/cell and enforce fencing at the recipient before the mutation.
...
- packages/runtime/src/cluster.ts:16-31
- packages/runtime/src/cluster.ts:145-157
...
- **CR-03.REGRESSION:** Independent processes, lease expiry, delayed holders, restarts and partitions; no stale owner may perform an accepted write. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
