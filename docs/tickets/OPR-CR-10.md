# OPR-CR-10 - Repair: Grouped aggregate results ignore the requested metric
...
Implement a typed aggregate algebra, explicit null/unit/decimal rules and correct grouped metrics; push supported queries to qualified storage.
...
- packages/runtime/src/oss.ts:140-160
...
- **CR-10.REGRESSION:** Independent exact oracle for grouped/ungrouped count/sum/average/min/max, empty sets, decimals, joins, pagination and access policies. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
