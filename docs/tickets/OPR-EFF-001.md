# OPR-EFF-001 - Lazy effect construction
...
Constructing a runtime operation must not mutate stores, claim approvals, acquire persistent resources or issue network calls.
...
- **EFF-001.T01:** Construct an update, proposal approval, ingestion and delivery effect without running them. **Expected:** Database, inbox, leases and outbound-call count remain unchanged.
- **EFF-001.T02:** Run the same effect twice with a fixed logical idempotency key. **Expected:** The operation obeys its documented replay semantics; laziness is not confused with one-time execution.
...
Requirement IDs: EFF-001. Acceptance case IDs: EFF-001.T01, EFF-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
