# OPR-SCN-002 - Broad domain-edit rights but immutable containment
...
A scenario explorer may freely alter authorized copied domain objects and assumptions; builders may alter draft code/schema. Neither can change real identity, enforcement, audit attestations, budgets or release controls.
...
- **SCN-002.T01:** Grant an explorer create/update/delete/link/reset rights for the whole authorized synthetic domain, without per-edit approval. **Expected:** All in-envelope edits run autonomously; the owner-controlled boundary remains unchanged.
- **SCN-002.T02:** Edit a copied PurposeDefinition or policy object, then try to use it as the real scenario grant or production policy. **Expected:** Modeled hypothetical policy changes affect only simulation semantics; real enforcement state is untouched.
...
Requirement IDs: SCN-002. Acceptance case IDs: SCN-002.T01, SCN-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
