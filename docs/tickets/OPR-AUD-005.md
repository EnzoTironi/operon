# OPR-AUD-005 - Replay without re-execution
...
Historical reconstruction never reissues remote actions, changes permissions or calls current models as a substitute for stored evidence.
...
- **AUD-005.T01:** Replay an externally delivered action while recording outbound calls and writes. **Expected:** Zero external commands or business transitions are generated.
- **AUD-005.T02:** Change facts, policies and model versions then replay. **Expected:** Original captured outputs and basis are reconstructed; missing artifacts are reported, not guessed.
...
Requirement IDs: AUD-005. Acceptance case IDs: AUD-005.T01, AUD-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
