# OPR-CR-04 - Repair: Lazy callback execution is not an untrusted-code sandbox
...
Keep this API as a trusted-model runner or add an actual restricted process/Wasm sandbox with limited host capabilities, memory/CPU limits and controlled egress.
...
- packages/runtime/src/sandbox.ts:49-113
...
- **CR-04.REGRESSION:** Run legitimate models and attempted filesystem/network/secret/global access under the real isolation technology; include blocking CPU work and interruption. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
