# OPR-PRO-003 - Current-state and aggregate conflict checks
...
Promotion resolves current production evidence, entitlements and aggregate constraints; old scenario data cannot overwrite intervening work or reserve stale capacity.
...
- **PRO-003.T01:** Change production stock or budget after scenario validation and promote concurrent plans sharing a quota. **Expected:** The runtime re-prepares or rejects stale plans and prevents write skew/over-allocation.
- **PRO-003.T02:** Use automatic rebase or a same-object revision match to conceal changed predicate dependencies. **Expected:** Dependency/predicate validation invalidates the stale approval; the whole relevant read set is checked.
...
Requirement IDs: PRO-003. Acceptance case IDs: PRO-003.T01, PRO-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
