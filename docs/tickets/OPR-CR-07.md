# OPR-CR-07 - Repair: Missing compensation can still be called compensated
...
Persist per-effect dispatch/acceptance/confirmation/unknown state. Missing, impossible and failed undo require explicit reconciliation and cannot be relabeled as successful compensation.
...
- packages/runtime/src/write-pipeline.ts:484-533
...
- **CR-07.REGRESSION:** Lost response after success, partial failure, no undo, undo failure, crash mid-recovery and independently observed external state. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
...
Requirement IDs: EXT-001. Acceptance case IDs: EXT-001.T01, EXT-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
