# OPR-UNI-006 - Accessible defaults and separately measured autonomy quality
...
The product supports a self-hosted core and synthetic scenario path without mandatory Palantir or paid model accounts; semantic sentinel claims require an actual configured model, while unsupported review routes remain explicit.
...
- **UNI-006.T01:** Install the selected local profile, exercise the non-model core and scenarios, then configure a supported local or hosted sentinel and evaluate it. **Expected:** No vendor account is needed for the deterministic core; live sentinel availability and cost are disclosed without pretending a rule stub is an LLM.
- **UNI-006.T02:** Disable the required review model, exhaust its budget or provide a disabled sentinel while advertising automated review. **Expected:** The profile reports unavailable capability and follows human/hold policy; it never silently converts missing review into clearance.
...
Requirement IDs: UNI-006. Acceptance case IDs: UNI-006.T01, UNI-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
