# V1-06 - Independent review and scenario containment
...
- `OPERON_SCENARIO_ROOT`
- `OPERON_SANDBOX_PROFILE`
- `OPERON_EVIDENCE_DIR`
...
```ts
review(req): Promise<ReviewResult>;
runScenario(manifest, candidate): Promise<ScenarioReceipt>;
```
...
- [x] required reviewer cannot be the acting principal.
- [x] network/filesystem/credential escape tests fail closed.
- [x] simulation never emits a production effect.
