# OPR-ARC-001 - Three parts and four planes
...
Separate declarative model assets, runtime enforcement and user/developer tools; security crosses data, logic and action interactions.
...
- **ARC-001.T01:** Ingest source evidence, query it, evaluate a function, propose/approve a change and observe writeback. **Expected:** Shared object IDs, versions, policies and decision lineage connect all three paths.
- **ARC-001.T02:** Try runtime writes through SDK setters, object services, maintenance APIs and database credentials issued to ordinary callers. **Expected:** Only governed writes or separately authorized/audited ingestion/admin paths can change authoritative state.
...
Requirement IDs: ARC-001. Acceptance case IDs: ARC-001.T01, ARC-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
