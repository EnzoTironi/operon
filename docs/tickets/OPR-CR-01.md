# OPR-CR-01 - Repair: Complete transaction and rollback correctness
...
One durable unit of work for object revisions, decision, approval consumption, budget reservation, dedupe and outbox. Remove application-level blind reversion as a substitute for atomic commit.
...
- packages/runtime/src/write-pipeline.ts:339-445
- packages/runtime/src/object-store.ts:111-126
...
- **CR-01.REGRESSION:** Real dependency and DB tests with failure/interruption/kill at every boundary; a competing valid write must never be erased. Inspect reopened DB and independent receipt store. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
...
Requirement IDs: TXN-001, EXT-001. Acceptance case IDs: TXN-001.T01, TXN-001.T02, BAI-037, BAI-038, BAI-039, BAI-040, BAI-041, BAI-042, EXT-001.T01, EXT-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
