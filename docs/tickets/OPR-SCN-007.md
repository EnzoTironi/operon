# OPR-SCN-007 - Egress isolation including model and connector paths
...
Scenario networking denies arbitrary external access; approved model/artifact services use an out-of-sandbox broker enforcing destination, purpose, payload and budget policy.
...
- **SCN-007.T01:** Execute scenario tools against permitted local simulators and a classified-data-compatible model gateway. **Expected:** Only declared destinations and allowed data flows are observed; credentials stay outside the agent workload.
- **SCN-007.T02:** Attempt production webhook, email, payment, DNS/HTTP redirect, metadata endpoint and nested-tool egress. **Expected:** Network and credential controls block unauthorized routes, even when the agent and sentinel request them.
...
Requirement IDs: SCN-007. Acceptance case IDs: SCN-007.T01, SCN-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
