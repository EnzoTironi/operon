# OPR-AUD-004 - Durability and non-repudiation scope
...
Persist audit with actions; protect critical signatures/anchors with an explicit trust and retention model.
...
- **AUD-004.T01:** Fail audit storage in the full action transaction. **Expected:** No business success without its required audit unit; operator sees the failure.
- **AUD-004.T02:** Verify signatures and external chain checkpoints with rotated/revoked keys and a truncated export. **Expected:** Invalid signatures/anchored omissions fail; a hash chain alone is not called administrator-proof immutability.
...
Requirement IDs: AUD-004. Acceptance case IDs: AUD-004.T01, AUD-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
