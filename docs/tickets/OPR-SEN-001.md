# OPR-SEN-001 - Independent evidence-bound review
...
The reviewer receives an authenticated, immutable review bundle containing task/grant/policy versions, evidence provenance, prepared effects and relevant execution history; proposer rationale is non-authoritative.
...
- **SEN-001.T01:** Prepare a registered action and independently resolve the bundle in the review service; compare every digest to authoritative storage. **Expected:** The review names the actual candidate, environment, evidence and intended effects; proposer-selected snippets are not the sole source.
- **SEN-001.T02:** Replace one target or evidence version in the agent summary while leaving the server-prepared bundle unchanged. **Expected:** The discrepancy is visible and cannot alter the authoritative review target.
...
Requirement IDs: SEN-001. Acceptance case IDs: SEN-001.T01, SEN-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
