# OPR-SCN-012 - Control audit and test attestations are outside agent writes
...
Explorer reset/delete rights affect working content only. Authoritative event records, baselines, signed results and independent expected outcomes are owned outside the mutable scenario.
...
- **SCN-012.T01:** Delete/reset all scenario working objects and reopen the independent run ledger. **Expected:** Baseline provenance, failed attempts, reset events and acceptance attestations still exist under retention policy.
- **SCN-012.T02:** Edit a local test report to PASS, change expected outcomes or ask the sentinel to certify it. **Expected:** The release validator trusts only independent candidate-bound evidence and detects the alteration.
...
Requirement IDs: SCN-012. Acceptance case IDs: SCN-012.T01, SCN-012.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
