# OPR-SEN-002 - Reviewer authority cannot expand execution authority
...
Sentinel may recommend proceed, hold, reject or more evidence, but cannot mint grants, impersonate humans, waive deterministic denial or execute business writes.
...
- **SEN-002.T01:** Inject a favorable sentinel verdict for a request forbidden by the task grant and call every production transport. **Expected:** The deterministic runtime denies it; no business state or outbound command changes.
- **SEN-002.T02:** Use sentinel credentials to approve as a human, change policy, edit evidence, expand a grant or call the executor. **Expected:** All such operations fail independently of the model response.
...
Requirement IDs: SEN-002. Acceptance case IDs: SEN-002.T01, SEN-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
