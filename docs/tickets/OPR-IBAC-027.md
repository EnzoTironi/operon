# OPR-IBAC-027 - Clock, retries and fault outcomes
...
Use controlled time for boundaries; separate business verdicts from operational faults and bound retried side effects.
...
- **IBAC-027-A:** Advance real Effect TestClock across expiry, freshness, backoff and lease boundaries; verify bounded worker concurrency. **Expected:** Behavior matches documented equality rules and schedules without wall-clock sleeping.
- **IBAC-027-B:** Make the verifier unavailable, produce DENY/UNKNOWN, or trigger repeated remote ambiguity under a generic retry policy. **Expected:** No fault or retry expands authority or repeats an unsafe side effect; policy verdicts are not retried as transient failures.
...
Requirement IDs: IBAC-027. Acceptance case IDs: IBAC-027-A, IBAC-027-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
