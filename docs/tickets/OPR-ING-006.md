# OPR-ING-006 - CROV Gate II is fact promotion
...
Before runtime facts enter a knowledge layer, evaluate Correct proxies, Consistent, Current and Complete; do not substitute the output-quality gate.

## Applicable contracts (fully stated)

- **S03:** Keep evidence, claims, admitted state and confirmed effects distinct; preserve attribution, conflicts, corrections and retractions.
...
- packages/runtime/src/funnel.ts
- packages/runtime/src/connectors.ts
- packages/schema/src/

## Steps

1. Resolve and freeze the relevant S03 contract; retain existing source vocabulary and open decisions.
...
- **ING-006.T01:** Promote batches with bad provenance/plausibility, contradictions, stale versions and missing coverage, one at a time. **Expected:** Each L1 dimension has its own result and policy-defined reject/review/downgrade disposition.
- **ING-006.T02:** Attempt to promote an unreviewed agent extraction or raw alarm statement as fact. **Expected:** Only admitted evidence crosses the promotion boundary; assertion provenance is retained.
...
Requirement IDs: ING-006. Acceptance case IDs: ING-006.T01, ING-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
