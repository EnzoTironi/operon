# OPR-EXT-002 - Transactional outbox delivery
...
Post-commit commands become durable with the local action and are delivered separately.
...
- **EXT-002.T01:** Commit and terminate before dispatch; restart a worker. **Expected:** Every committed command is recoverable; uncommitted commands are never sent.
- **EXT-002.T02:** Run with no valid handler or unavailable destination. **Expected:** Command remains pending/blocked with a visible reason, not successfully delivered.
...
Requirement IDs: EXT-002. Acceptance case IDs: EXT-002.T01, EXT-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
