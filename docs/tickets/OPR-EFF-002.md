# OPR-EFF-002 - Explicit live and test services
...
Provide storage, policy, authentication, time, identifiers and external clients through explicit services/layers without hidden authority globals.
...
- **EFF-002.T01:** Build and execute a program with a required service omitted. **Expected:** Compilation or startup fails explicitly; it cannot fall back to permissive auth or an ephemeral production store.
- **EFF-002.T02:** Run two scoped runtimes with different tenants, clocks, keys and databases. **Expected:** Services and mutable state cannot bleed across runtimes or tests.
...
Requirement IDs: EFF-002. Acceptance case IDs: EFF-002.T01, EFF-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
