# OPR-SCN-011 - Lifecycle, revocation and cancellation cover detached work
...
Scenario expiry, owner termination and access revocation stop new work, cancel descendants and fence stale workers; retained data and audit follow explicit separate policies.
...
- **SCN-011.T01:** Expire or revoke a scenario during parallel model calls, calculations and delayed jobs using a controlled clock. **Expected:** No new writes occur after the allowed cutoff; descendants stop and stale leases cannot resume the scenario.
- **SCN-011.T02:** Restart a worker after TTL, replay a late callback and attempt access through a saved snapshot after revocation. **Expected:** Scope/lease/revocation checks reject future operations; already disclosed knowledge is not claimed to be retractable.
...
Requirement IDs: SCN-011. Acceptance case IDs: SCN-011.T01, SCN-011.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
