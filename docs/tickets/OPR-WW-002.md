# OPR-WW-002 - Decision evidence and calibrated sensors
...
Declare N readings/two-hour coverage, DO freshness, calibration and lab-data budgets with explicit fallback behavior.
...
- D08 [OPEN]: Have the domain owner supply R(d), units, sampling/coverage predicates, timezone, clock tolerance, freshness inequalities and exemption policy before executing boundary tests. Keep teaching fixtures separate from operational policies.
...
- **WW-002.T01:** Use complete, sparse, missing, stale and conflicting telemetry/lab evidence with a fixed clock. **Expected:** Standard operation passes; calibration request, alarm-only or labeled mandatory review follows the specific deficiency.
- **WW-002.T02:** Test day 30 and just beyond; 15-minute freshness and just beyond; absent target relationships. **Expected:** Declared closed time boundary and missing-target handling are enforced.
...
Requirement IDs: WW-002. Acceptance case IDs: WW-002.T01, WW-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
