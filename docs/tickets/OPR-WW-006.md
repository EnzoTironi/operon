# OPR-WW-006 - Permit-time attribution and five test classes
...
Bind compliance to sampling-time permit versions and retain positive, boundary, missing, conflict and temporal regression cases.
...
- **WW-006.T01:** Sample before a renewal, receive the lab result afterward and replay later. **Expected:** Compliance uses the sampling-time version, not the newest registered version.
- **WW-006.T02:** Encounter a renewed but not yet registered permit in the fixture. **Expected:** Use and label the applicable registered version per the resolved source policy; unknown applicability is not guessed.
...
Requirement IDs: WW-006. Acceptance case IDs: WW-006.T01, WW-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
