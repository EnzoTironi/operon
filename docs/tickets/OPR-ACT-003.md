...
Support manual, automatic, recommendation-confirmation and rejected/degraded paths without conflating them with agent tiers.
...
- **ACT-003.T01:** Exercise one approved action definition in each supported mode. **Expected:** Execution and required human role follow the declared mode; a label does not waive guards.
- **ACT-003.T02:** Change an action from confirmation to automatic mode. **Expected:** Versioned review and evidence are required; old unapproved proposals are not silently promoted.
...
Requirement IDs: ACT-003. Acceptance case IDs: ACT-003.T01, ACT-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
