# OPR-AUD-003 - Cryptographic chaining and isolation
...
Use a genuine cryptographic digest and prevent mutation through input/output aliasing; serialize concurrent chain appends.
...
- **AUD-003.T01:** Re-run Aa/BB collision and nested input/get/list mutation cases against shipped audit storage. **Expected:** No collision from the old rolling-hash defect; external mutation cannot alter stored records.
- **AUD-003.T02:** Concurrently append, tamper with content/order/linkage, remove middle records and reload. **Expected:** Valid serialization forms one verifiable chain; supported tampering is detected. Tail deletion needs an external anchor to be detectable.
...
Requirement IDs: AUD-003. Acceptance case IDs: AUD-003.T01, AUD-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
