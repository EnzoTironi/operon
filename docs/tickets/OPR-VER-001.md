# OPR-VER-001 - Gate I compilation fidelity
...
For a declared shared fragment, check the stated inclusion/equivalence relation between source policy and compiled guards.
...
- **VER-001.T01:** Mutate a threshold, delete a condition or alter priority; run the actual checker. **Expected:** A soundness check finds source-deny/target-allow counterexamples when present; corpus regression alone is not called an SMT proof.
- **VER-001.T02:** Present a successful result with its universe, closure, abstraction, solver and direction of inclusion. **Expected:** Certificate states exactly what is established; unknown/timeout is not success.
...
Requirement IDs: VER-001. Acceptance case IDs: VER-001.T01, VER-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
