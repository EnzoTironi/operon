# OPR-IBAC-005 - Subject and delegation chain
...
Requester, delegator, agent, reviewer and executor remain distinct; delegation is limited, audience-bound and revocable.
...
- **IBAC-005-A:** Delegate a subset of a human grant to an agent and execute within the declared resource audience. **Expected:** The receipt preserves all actors and the subset restrictions.
- **IBAC-005-B:** Replay the delegation at another resource server or replace an actor/reviewer ID in MCP arguments. **Expected:** The request fails trusted identity and audience checks; it cannot become human approval.
...
Requirement IDs: IBAC-005. Acceptance case IDs: IBAC-005-A, IBAC-005-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
