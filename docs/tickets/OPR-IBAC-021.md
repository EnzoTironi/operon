# OPR-IBAC-021 - Attempt accounting and protected audits
...
Define durable capture for successful, denied, failed and unresolved attempts plus access controls and bounded completeness claims.
...
- **IBAC-021-A:** Generate each outcome type, then export an authorized interval with counts and ordering tied to the capture contract. **Expected:** Attempt records and business receipts are distinguishable and reconciled to the defined event source.
- **IBAC-021-B:** Drop a denied event, expose a sensitive justification to another purpose, or call inclusion proofs evidence of complete filtered results. **Expected:** The capture discrepancy/access violation is detected; query-completeness claims remain limited to actual supported evidence.
...
Requirement IDs: IBAC-021. Acceptance case IDs: IBAC-021-A, IBAC-021-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
