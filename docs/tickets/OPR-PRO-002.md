# OPR-PRO-002 - Immutable promotion manifest and reproducible candidate
...
The proposal manifest binds scenario/baseline, selected changes, task/grant, code/schema/policy/model versions, evaluations, reviewer findings and target environment.
...
- **PRO-002.T01:** Rebuild a selected candidate from its manifest and independently compare digests. **Expected:** Inputs, artifacts, scope and review applicability are reproducible and attributable.
- **PRO-002.T02:** Modify one recipient, action parameter, code artifact, model version or selected subset after review. **Expected:** The manifest changes and dependent review/approval evidence is invalidated.
...
Requirement IDs: PRO-002. Acceptance case IDs: PRO-002.T01, PRO-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
