# OPR-VAL-005 - Authenticated CLI and explicit demo mode
...
Production CLI and MCP startup use trusted credentials and scopes; identity/role/tier flags and direct writes are limited to an explicit synthetic or privileged-maintenance profile.
...
- **VAL-005.T01:** Attempt to set --subject-type user --role reviewer without credentials **Expected:** Production operation is denied independently of supplied flags
- **VAL-005.T02:** Run authorized device/session flow and explicitly isolated demo flow **Expected:** Legitimate work succeeds; demo authority cannot target production
...
Requirement IDs: VAL-005. Acceptance case IDs: VAL-005.T01, VAL-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
