# OPR-IBAC-016 - Revocation and policy changes
...
Expiry, revocation and policy changes are rechecked under an explicit consistency contract, including external policy providers.
...
- **IBAC-016-A:** Reevaluate a prepared action across a documented compatible policy update and valid grant lease. **Expected:** The decision records the actual version/epoch used and permitted compatibility path.
- **IBAC-016-B:** Revoke the delegator/reviewer or grant during review or race a policy epoch change against commit. **Expected:** The required atomic/lease check blocks execution or reports its explicitly bounded semantics; no unsupported instant-revocation claim.
...
Requirement IDs: IBAC-016. Acceptance case IDs: IBAC-016-A, IBAC-016-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
