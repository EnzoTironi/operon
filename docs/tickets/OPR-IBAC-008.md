# OPR-IBAC-008 - Typed versioned evidence
...
Claims resolve tenant, type, ID, version, property, valid/recorded time and actual source span/hash when cited.
...
- **IBAC-008-A:** Resolve a supported claim against a pinned source span and matching target relationship; round-trip the references. **Expected:** The checker demonstrates the exact supporting fact and version, not merely presence of an ID.
- **IBAC-008-B:** Reuse an ID from another type/tenant, cite a nonexistent span, change sourceVersion, or cite the wrong beneficiary. **Expected:** The claim is unsupported or unauthorized and cannot justify execution.
...
Requirement IDs: IBAC-008. Acceptance case IDs: IBAC-008-A, IBAC-008-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
