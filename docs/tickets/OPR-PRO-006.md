# OPR-PRO-006 - Data actions and definition releases remain distinct
...
Data promotions use business-action contracts; schema, function, policy, model and agent changes use separately versioned build/migration/release workflows with independent tests.
...
- **PRO-006.T01:** Promote a data plan and a policy-code change via their respective reviewed workflows. **Expected:** Each receives the right tests, migration impact analysis and permissions; neither silently piggybacks on the other.
- **PRO-006.T02:** Smuggle an authorization rule change into a data diff or execute migration scripts from an untrusted scenario attachment. **Expected:** The route/type mismatch is rejected before execution.
...
Requirement IDs: PRO-006. Acceptance case IDs: PRO-006.T01, PRO-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
