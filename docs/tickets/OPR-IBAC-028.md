# OPR-IBAC-028 - Policy fidelity and bounded proofs
...
Scope any formal claim to an explicit model, policy fragment, assumptions, abstraction, engine and bound; unknown is not pass.
...
- **IBAC-028-A:** Check non-expansion/effect containment/budget invariants on a small stated model and validate counterexamples against an independent oracle. **Expected:** The result states exactly what was established and its assumptions; execution semantics match the checked model fragment.
- **IBAC-028-B:** Mutate the compiled guard to admit a forbidden state, omit an external effect or time out the checker. **Expected:** A counterexample or UNKNOWN/BLOCKED is reported; regression success never substitutes silently for a proof.
...
Requirement IDs: IBAC-028. Acceptance case IDs: IBAC-028-A, IBAC-028-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
