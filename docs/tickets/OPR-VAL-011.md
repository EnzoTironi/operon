# OPR-VAL-011 - Signed candidate-bound evidence gate
...
A protected evaluator must verify exact candidate/tree/lock/catalogue/harness/policy/config/fixtures, trusted runner attestation, case coverage and artifact bytes.
...
- **VAL-011.T01:** Forge PASS, omit/duplicate a case, alter hash or use a candidate-owned signing key **Expected:** Gate rejects every untrusted/incomplete binding
- **VAL-011.T02:** Supply a complete signed independent-fixture run and then corrupt its receipt **Expected:** Valid run qualifies for independent review only; corrupt evidence is rejected
...
Requirement IDs: VAL-011. Acceptance case IDs: VAL-011.T01, VAL-011.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
