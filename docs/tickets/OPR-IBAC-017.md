# OPR-IBAC-017 - Complete transaction and receipt
...
Business edits, decision, approval consumption, grant usage, budget and outbox share one required commit boundary.
...
- **IBAC-017-A:** Commit a valid action and inspect all participating records through a second connection. **Expected:** All records agree on one logical transition and idempotency result.
- **IBAC-017-B:** Inject failure or kill the process at every write boundary; reopen and retry the same logical request. **Expected:** Either the entire local transition exists or none does; no spent grant without its corresponding action or duplicate transition.
...
Requirement IDs: IBAC-017. Acceptance case IDs: IBAC-017-A, IBAC-017-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
