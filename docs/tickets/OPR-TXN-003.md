# OPR-TXN-003 - Create/update and revision checks
...
Distinguish create from update and enforce optimistic concurrency without version-1 escape paths.
...
- **TXN-003.T01:** Attempt to write version 1 over an existing later revision. **Expected:** Conflict, never replacement/reset of the revision.
- **TXN-003.T02:** Race updates based on the same object revision via separate database connections. **Expected:** Only allowed serializable outcomes commit; stale contenders fail or reevaluate.
...
Requirement IDs: TXN-003. Acceptance case IDs: TXN-003.T01, TXN-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
