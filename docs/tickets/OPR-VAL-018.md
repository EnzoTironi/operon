# OPR-VAL-018 - Immutable handler restoration
...
Restoring an approval requires the exact pinned action release; unknown IDs or changed implementations cannot fall back to a different registered action.
...
- **VAL-018.T01:** Change persisted proposal actionTypeId to an unknown ID **Expected:** Restore is rejected/quarantined without executing another action
- **VAL-018.T02:** Load the known pinned action and attempt a later implementation swap **Expected:** Valid release works once; swapped behavior invalidates approval
...
Requirement IDs: VAL-018. Acceptance case IDs: VAL-018.T01, VAL-018.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
