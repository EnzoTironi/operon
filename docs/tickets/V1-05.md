...
- `packages/runtime/src/reconciliation/`
- `tests/execution/`
...
- [ ] commit plus outbox enqueue is atomic.
- [ ] same idempotency key plus changed input conflicts.
- [ ] crash at every fault point yields one effect or explicit UNKNOWN then reconciled truth.
