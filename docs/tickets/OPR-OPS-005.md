# OPR-OPS-005 - Adversarial input and network boundaries
...
Protect public inputs, rendering, SQL, file/URL access and connector destinations against injection and authority confusion.
...
- **OPS-005.T01:** Submit SQL/prototype/path/HTML injection and malformed nested schemas through real entrypoints. **Expected:** No unauthorized execution, data exposure or unbounded parse behavior.
- **OPS-005.T02:** Attempt connector/schema retrieval to disallowed internal/file endpoints and redirects. **Expected:** Egress and redirect policy is enforced; credentials are not forwarded to untrusted destinations.
...
Requirement IDs: OPS-005. Acceptance case IDs: OPS-005.T01, OPS-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
