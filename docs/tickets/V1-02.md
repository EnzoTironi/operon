# V1-02 - Canonical evidence and bitemporal state
...
```ts
admit(envelope, principal): Promise<AdmissionReceipt>;
query(set, asOf:{valid:string;transaction:string}): Promise<QueryReceipt>;
```
...
- [x] conflicting evidence remains attributed and unresolved.
- [x] valid-time correction does not rewrite transaction history.
- [x] independent SQL/oracle matches API result.
