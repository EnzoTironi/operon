# OPR-SEN-004 - Required-review outage and freshness semantics
...
Versioned policy declares where review is mandatory; timeout, model unavailability or stale review cannot silently satisfy it. Review cache keys bind all decision-relevant versions and environment.
...
- **SEN-004.T01:** Run required-review requests with TestClock at expiry boundaries, including policy, recipient and model changes. **Expected:** Only a still-applicable review can be used; changed intent or required versions trigger a new review.
- **SEN-004.T02:** Interrupt the reviewer, exhaust its budget and replay a review from another environment; then run an explicitly advisory synthetic experiment. **Expected:** Required paths remain uncommitted; advisory work may continue only with a visible UNREVIEWED marker, never a fabricated pass.
...
Requirement IDs: SEN-004. Acceptance case IDs: SEN-004.T01, SEN-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
