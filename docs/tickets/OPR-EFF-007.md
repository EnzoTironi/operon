...
Retry only declared transient operations and preserve logical request IDs and authority checks.
...
- **EFF-007.T01:** Inject invalid credentials, business denial, lock conflict and transient delivery failure. **Expected:** Permanent failures do not retry; transient retry count/backoff matches configuration.
- **EFF-007.T02:** Allow a remote operation to accept, then lose its response and trigger interruption/retry. **Expected:** An uncertain non-idempotent effect enters reconciliation instead of blind re-execution.
...
Requirement IDs: EFF-007. Acceptance case IDs: EFF-007.T01, EFF-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
