# OPR-API-002 - HTTP contract and startup safety
...
Validate request/response schemas, error semantics, body limits and production configuration using the actual server.
...
- **API-002.T01:** Send malformed/oversized JSON, unsupported methods/content types and unexpected authority fields. **Expected:** Bounded failures with clear status and no mutations or secret leaks.
- **API-002.T02:** Start with missing database/auth/secret settings and deliberately demo-only settings. **Expected:** Normal deployment fails safely; demo mode is explicit and isolated.
...
Requirement IDs: API-002. Acceptance case IDs: API-002.T01, API-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
