# OPR-SCN-014 - Simulated time and real security time remain separate
...
Simulation clocks can move backward or forward for business modeling; grant expiry, credential validity, real budgets and retention are governed by a trusted non-agent-controlled clock.
...
- **SCN-014.T01:** Time-travel a simulation across policy/model versions while observing real authorization and resource expiry. **Expected:** Hypothetical temporal results are reproducible without changing security lifetimes.
- **SCN-014.T02:** Set simulated time before token issuance or TTL expiry and retry a revoked/expired grant. **Expected:** The real control-plane clock and current authority reject the request.
...
Requirement IDs: SCN-014. Acceptance case IDs: SCN-014.T01, SCN-014.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
