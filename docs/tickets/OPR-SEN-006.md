# OPR-SEN-006 - Reviewer data minimization and output confidentiality
...
Review access is purpose-scoped and separately authorized; broader reviewer visibility never flows to the proposer through explanations, logs, model providers or aggregate findings.
...
- **SEN-006.T01:** Review a case requiring an internal restricted fact and request explanations under two differently entitled actors. **Expected:** The reviewer can use authorized evidence while responses reveal only permitted information and controlled reason codes.
- **SEN-006.T02:** Attempt to expose a hidden field via rationale, evidence URLs, cross-case memory or an unapproved model endpoint. **Expected:** The output/provider boundary blocks the disclosure and records a protected event.
...
Requirement IDs: SEN-006. Acceptance case IDs: SEN-006.T01, SEN-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
