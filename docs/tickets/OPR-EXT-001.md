# OPR-EXT-001 - Distinct webhook semantics
...
Support explicitly declared synchronous pre-edit writeback and post-edit side-effect semantics without claiming distributed rollback.
...
- **EXT-001.T01:** Have the authoritative external endpoint reject a writeback command. **Expected:** No success ontology transition is recorded; an actionable failure/attention state remains.
- **EXT-001.T02:** Let writeback accept, then fail local commit or lose the result. **Expected:** Record/recover an uncertain commitment and reconcile; never assert remote rollback from local failure.
