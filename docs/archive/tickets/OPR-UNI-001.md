# OPR-UNI-001 - Single source-preserving requirement ledger
...
The consolidation retains all 171 base and 30 IBAC requirement/test IDs and original meanings, labels new recommendations and maps every item to a milestone and release profile.
...
- **UNI-001.T01:** Validate ID sets, original record equality, source namespaces, mappings and decision references against preserved inputs. **Expected:** No source requirement or planned test disappears; overlaps are linked rather than counted as unique independent guarantees.
- **UNI-001.T02:** Delete one legacy requirement, rename a test or mark a dependency-blocked item complete. **Expected:** The structural/release checks detect the issue; plan consistency is not reported as application correctness.
...
Requirement IDs: UNI-001. Acceptance case IDs: UNI-001.T01, UNI-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
