# OPR-AUTH-001 - Fail-closed HTTP authentication
...
Every protected route requires an actual configured verifier; no JSON/unsigned-JWT/default-human identity fallback exists.
...
- **AUTH-001.T01:** Call actual protected routes with raw JSON admin claims, unsigned expired JWT, empty JSON, malformed or absent credentials. **Expected:** All fail authentication; body/header role assertions never create trusted authority.
- **AUTH-001.T02:** Start the production handler without an authentication service. **Expected:** Startup or protected requests fail closed, not test-mode success.
