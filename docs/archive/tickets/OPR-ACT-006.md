# OPR-ACT-006 - Governed composition
...
Each workflow step and compensation has its own authorization, guard, durable status and audit record.
...
- **ACT-006.T01:** Execute two committed steps then fail the third. **Expected:** Earlier history remains; declared compensation runs in reverse order rather than pretending global rollback.
- **ACT-006.T02:** Compose an irreversible step before reversible work. **Expected:** Design validation flags/rejects unsafe ordering or requires explicit reviewed justification.
...
Requirement IDs: ACT-006. Acceptance case IDs: ACT-006.T01, ACT-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
