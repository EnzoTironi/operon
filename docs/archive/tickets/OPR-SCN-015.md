# OPR-SCN-015 - Evidence quality of simulations is explicitly limited
...
Runs record model applicability, unsupported operations, uncertainty, coverage and deterministic inputs/seeds; a copied state is not itself a predictive or causal model.
...
- **SCN-015.T01:** Compare alternatives under a documented simulator with fixed input versions and report its assumptions and covered effects. **Expected:** Results distinguish model predictions from actual observations and list unsupported downstream effects.
- **SCN-015.T02:** Use an out-of-envelope input, missing physical model or nondeterministic provider result while claiming exact replay. **Expected:** The report is marked unsupported/uncertain; captured model outputs can be replayed without claiming new calls reproduce them.
...
Requirement IDs: SCN-015. Acceptance case IDs: SCN-015.T01, SCN-015.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
