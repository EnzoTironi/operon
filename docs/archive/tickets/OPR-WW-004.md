# OPR-WW-004 - Propose/approve/apply and countersign
...
Enforce distinct operator/engineer/manager roles, current evidence and declaration of external command status.
...
- **WW-004.T01:** Test exactly 20% and just above with and without manager countersignature. **Expected:** Only the above-threshold change requires that additional signature under the chapter’s stated policy.
- **WW-004.T02:** Recover DO or change sensor state between proposal and confirmation; race two unresolved proposals. **Expected:** Revalidation/unique-active-proposal policy prevents stale or conflicting execution.
...
Requirement IDs: WW-004. Acceptance case IDs: WW-004.T01, WW-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
