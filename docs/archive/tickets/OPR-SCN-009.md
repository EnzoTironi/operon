# OPR-SCN-009 - Executable builders have a tested OS isolation boundary
...
Generated code runs without host administrator access, shared production mounts or control sockets, in a supported tested sandbox. TypeScript types and Effect Layers alone are not an isolation boundary.
...
- D13 [OPEN]: Review and approve an explicit versioned contract; the proposed default is not recorded as approved.
...
- **SCN-009.T01:** Run arbitrary fixture programs in the chosen process/container/VM profile and inspect filesystem, privileges and network from inside. **Expected:** Only declared scenario resources are accessible; no production capabilities are linked into the workload.
- **SCN-009.T02:** Try child processes, symlinks, filesystem escape paths, host sockets and process-level direct network APIs. **Expected:** The supported isolation mechanism enforces the boundary outside the language runtime.
...
Requirement IDs: SCN-009. Acceptance case IDs: SCN-009.T01, SCN-009.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
