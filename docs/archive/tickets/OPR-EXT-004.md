# OPR-EXT-004 - Fenced worker claims
...
Prevent expired workers from committing delivery ownership/results after a lease has been reassigned.
...
- **EXT-004.T01:** Pause worker A, expire its lease and let B claim; resume A. **Expected:** A’s stale fencing token cannot overwrite B’s authoritative delivery state.
- **EXT-004.T02:** Kill a worker after remote acceptance but before delivery acknowledgment. **Expected:** Recovery uses receiver idempotency/reconciliation; lease exclusion alone is not claimed to prevent duplicate network effects.
...
Requirement IDs: EXT-004. Acceptance case IDs: EXT-004.T01, EXT-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
