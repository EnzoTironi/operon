# OPR-TIM-005 - Derived evidence freshness and completeness
...
Track dependencies and explicit R(d); evaluate required coverage, currentness and not-applicable exemptions.
...
- **TIM-005.T01:** Compute a fresh-looking output from one stale required input. **Expected:** The derived evidence cannot hide the stale dependency.
- **TIM-005.T02:** Exercise missing, unknown, not applicable and sparse-window evidence under a declared R(d). **Expected:** Each receives its declared treatment; a populated field is not proof of complete coverage.
...
Requirement IDs: TIM-005. Acceptance case IDs: TIM-005.T01, TIM-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
