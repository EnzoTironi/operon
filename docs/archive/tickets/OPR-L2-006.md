# OPR-L2-006 - Runtime integration of quality gates
...
Apply configured output and admission checks at the real agent/tool/inbox boundaries, not only in helper-unit tests.
...
- **L2-006.T01:** Send deliberately uncited/incomplete output through the actual generation or tool response path. **Expected:** It cannot bypass the configured gate merely because a direct helper test passes.
- **L2-006.T02:** Fail the output/admission validation service. **Expected:** The system reports unverified/unavailable or routes review; it does not silently emit a verified badge.
...
Requirement IDs: L2-006. Acceptance case IDs: L2-006.T01, L2-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
