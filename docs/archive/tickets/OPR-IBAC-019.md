# OPR-IBAC-019 - Bitemporal decision reconstruction
...
Replay distinguishes contemporaneous recorded knowledge from later valid-time corrections and never reissues effects.
...
- **IBAC-019-A:** Replay an old authorized action before and after a retroactive evidence correction. **Expected:** The original basis is preserved and later corrections appear separately with their recorded time.
- **IBAC-019-B:** Overwrite current evidence or use the latest policy/model during purported historical replay. **Expected:** The mismatch is exposed; the system never reports the changed basis as what was known originally.
...
Requirement IDs: IBAC-019. Acceptance case IDs: IBAC-019-A, IBAC-019-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
