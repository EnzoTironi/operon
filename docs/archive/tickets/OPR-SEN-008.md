# OPR-SEN-008 - Held-out live-model evaluation and correlated-failure measurement
...
The advertised sentinel must be evaluated using a real model/provider configuration, independent labels, clean and adversarial tasks and repeated trials; different agents are not assumed independent.

## Applicable contracts (fully stated)

- **S09:** Semantic review is independent and may not issue policy or authority; unavailability and injection fail safely.

## Ticket-specific decisions

- D14 [OPEN]: Review and approve an explicit versioned contract; the proposed default is not recorded as approved.

## Exact implementation locations

- proposed packages/sentinel/
- packages/runtime/src/verification.ts

## Steps

1. Resolve and freeze the relevant S09 contract; retain existing source vocabulary and open decisions.
...
- **SEN-008.T01:** Compare deterministic-only, sentinel-shadow and sentinel-gated variants on frozen held-out tasks; stratify by risk/domain and record model/settings. **Expected:** Report unsafe authorization, unsafe execution, false holds, task completion, human load, latency, cost and uncertainty with sample counts.
- **SEN-008.T02:** Use same-model and diverse-model proposer/reviewer pairs, adaptive rephrasing and paraphrased repeats. **Expected:** Correlated misses and reviewer-shopping are measured; no first-try success or zero-observed-failure result is called a universal guarantee.
...
Requirement IDs: SEN-008. Acceptance case IDs: SEN-008.T01, SEN-008.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
