# OPR-SCN-013 - Scenario identity is visible in every output surface
...
UI, CLI, API, exports, tool results and telemetry identify hypothetical versus observed data, scenario/baseline and simulation status. Loss of labels never implies production truth.
...
- **SCN-013.T01:** Open, export and share a scenario plan in all supported interfaces, including accessibility modes. **Expected:** Environment and hypothesis labels remain perceivable; comparison shows modeled assumptions separately from observed facts.
- **SCN-013.T02:** Strip labels or paste hypothetical evidence into a production intake. **Expected:** Provenance checks prevent automatic admission as observed evidence and require a distinct accountable admission path.
...
Requirement IDs: SCN-013. Acceptance case IDs: SCN-013.T01, SCN-013.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
