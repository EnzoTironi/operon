...
Compare local commitment state with external observations and route discrepancies to an accountable queue.
...
- **EXT-006.T01:** Change the remote value outside the platform and run scheduled reconciliation. **Expected:** Mismatch, evidence, owner and disposition are recorded.
- **EXT-006.T02:** Receive delayed external acknowledgment/telemetry. **Expected:** Local status advances only on a matching trustworthy observation; repeated observations are idempotent.
...
Requirement IDs: EXT-006. Acceptance case IDs: EXT-006.T01, EXT-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
