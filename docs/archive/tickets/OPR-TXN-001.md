# OPR-TXN-001 - Required complete unit of work
...
Atomically commit business changes, links, decision/action logs, approval consumption, idempotency state and outbound-command records.
...
- **TXN-001.T01:** Fail after each participant write and immediately before commit through the real adapter. **Expected:** Either the whole success unit exists or none of it does; a separate failure dossier may be appended without claiming success.
- **TXN-001.T02:** Supply an adapter that cannot provide the required transaction contract. **Expected:** Startup/action registration refuses governed mutation instead of falling back to per-object writes.
...
Requirement IDs: TXN-001. Acceptance case IDs: TXN-001.T01, TXN-001.T02, BAI-037, BAI-038, BAI-039, BAI-040, BAI-041, BAI-042. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
