# OPR-PRO-001 - Promotion is a new production proposal
...
A selected scenario creates a new reviewed production action or plan; the scenario database, grants, approvals, audit rows, assumptions and execution receipts are never bulk copied into production.
...
- **PRO-001.T01:** Select a valid inventory-reallocation outcome and generate a production proposal from registered business intent. **Expected:** Only permitted actionable changes are proposed; current production policy and evidence are resolved afresh.
- **PRO-001.T02:** Include copied grants, fake observations, simulated payments or model assumptions in the selected diff. **Expected:** Protected/non-actionable artifacts are excluded or rejected with explicit reasons.
...
Requirement IDs: PRO-001. Acceptance case IDs: PRO-001.T01, PRO-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
