# OPR-UX-003 - Accessibility and usable local deployment
...
Support keyboard navigation, readable labels/focus, non-color-only status, responsive evidence views and an independently usable local workflow.
...
- **UX-003.T01:** Complete login, evidence inspection and approval using keyboard and screen-reader checks. **Expected:** No blocked focus trap or unlabeled essential action; stale/error status has textual meaning.
- **UX-003.T02:** Run quickstart and evidence-review tasks with representative users on declared minimum hardware/viewport. **Expected:** Task outcomes and usability failures are measured; no universal accessibility claim from a screenshot.
...
Requirement IDs: UX-003. Acceptance case IDs: UX-003.T01, UX-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
