# OPR-HC-002 - Declared R(d) and preferred fallback
...
Require the stated evidence contract; missing meal evidence proposes its recording, then escalates after the supplementation timeout.
...
- **HC-002.T01:** Omit a required same-day observation. **Expected:** A recording proposal is issued; no silently generated dosage-class recommendation or label-only bypass.
- **HC-002.T02:** Admit evidence before timeout or advance beyond the chapter’s two-hour backstop. **Expected:** Early admission revalidates/revives with new evidence; timeout suspends and routes a missing-basis case to a clinician.
...
Requirement IDs: HC-002. Acceptance case IDs: HC-002.T01, HC-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
