# OPR-TIM-004 - Decision-relative freshness
...
Measure state age from the relevant observation’s valid time and a versioned budget; do not use unrelated recent updates to refresh evidence.
...
- **TIM-004.T01:** Ingest an old observation now and update an unrelated property. **Expected:** The old required observation remains stale despite recent transaction/object timestamps.
- **TIM-004.T02:** Use the same evidence for two decisions with different freshness budgets. **Expected:** One may pass and the other fail; the result identifies the decision and budget version.
...
Requirement IDs: TIM-004. Acceptance case IDs: TIM-004.T01, TIM-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
