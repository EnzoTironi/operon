# OPR-EFF-004 - Typed operational failures
...
Distinguish validation, authorization, concurrency, storage, verifier and delivery failures; do not turn failure into successful empty output.
...
- **EFF-004.T01:** Inject each declared failure through the real public action call. **Expected:** The typed error/category, disposition and audit behavior match the contract; no catch-all ALLOW or success.
- **EFF-004.T02:** Throw an unexpected defect in a guard or mutation handler. **Expected:** It cannot authorize or partially commit; diagnostics preserve cause and omit secrets.
...
Requirement IDs: EFF-004. Acceptance case IDs: EFF-004.T01, EFF-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
