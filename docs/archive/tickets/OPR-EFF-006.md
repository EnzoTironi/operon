# OPR-EFF-006 - Clock-driven policy and scheduling tests
...
Use an injectable clock for freshness, expiry, leases, backoff and obligations; test against the selected Effect version.
...
- **EFF-006.T01:** Advance the test clock to just below, exactly at and above each configured boundary. **Expected:** Declared inclusive/exclusive behavior holds without sleeping in unit tests.
- **EFF-006.T02:** Change the test clock while the real pipeline checks freshness and expiry. **Expected:** All relevant checks use that clock; hidden Date.now calls cannot bypass the test or policy.
...
Requirement IDs: EFF-006. Acceptance case IDs: EFF-006.T01, EFF-006.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
