# OPR-WW-007 - Simulation and live-control claims
...
If claiming the chapter’s plant-model/research or real integration capability, validate an actual simulator/plant model and separately validate control-system adapters.
...
- **WW-007.T01:** Run the selected BSM1/plant-model scenarios with provenance, parameters and reproducible outputs. **Expected:** Physical-response validation and safety margins are reported; scalar mock guards are not called a plant simulator.
- **WW-007.T02:** Exercise an isolated test control-system endpoint and obtain qualified engineering signoff before any field trial. **Expected:** Simulated/test/live status is explicit; no claimed operational safety from synthetic fixtures alone.
...
Requirement IDs: WW-007. Acceptance case IDs: WW-007.T01, WW-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
