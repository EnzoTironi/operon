# OPR-VAL-009 - Executable documentation and package scripts
...
Documented commands, flags, scripts, Node profile and tool names must match the shipped registry and be tested from a clean packaged installation.
...
- **VAL-009.T01:** Execute every verify-operon copy/paste command with --help and fixtures **Expected:** Missing verify:all/run operon scripts and unknown commands block documentation gate
- **VAL-009.T02:** Check SQLite profile on supported and unsupported Node API availability **Expected:** Profile requirements are detected, not inferred from Node >=20 alone
...
Requirement IDs: VAL-009. Acceptance case IDs: VAL-009.T01, VAL-009.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
