# OPR-UNI-004 - Real Effect services plus enforceable environment separation
...
Live, scenario and test services have explicit supported wiring and scoped lifetimes; every shipped adapter is tested with the selected real Effect release and external isolation where untrusted code runs.
...
- **UNI-004.T01:** Compile positive/negative wiring fixtures and run actual Effect services through cancellation, nested work, rebase and persistent commits. **Expected:** No live dispatcher is usable in a scenario runtime; construction is lazy and cleanup respects the commit boundary.
- **UNI-004.T02:** Attempt a TypeScript cast, direct low-level call or child process to obtain a production service. **Expected:** Runtime identity and OS boundaries still refuse it; types and dependency injection are not the only defense.
...
Requirement IDs: UNI-004. Acceptance case IDs: UNI-004.T01, UNI-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
