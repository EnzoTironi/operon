# OPR-FUN-001 - Typed, versioned read functions
...
Derived values and query functions use declared input/output types, caller permissions and reproducible logic versions.
...
- **FUN-001.T01:** Evaluate a pure function twice on identical pinned inputs and version. **Expected:** Results match; no persistent state or external effect changes.
- **FUN-001.T02:** Invoke a function on forbidden inputs or return an invalid result. **Expected:** Permission/validation fails before unapproved derived data is exposed.
...
Requirement IDs: FUN-001. Acceptance case IDs: FUN-001.T01, FUN-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
