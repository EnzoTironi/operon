# OPR-HC-005 - Boundary-only verifier and contradictions
...
Verifier checks declared contraindication/ceiling/step/evidence boundaries, never computes a dose; conflicting patient/device evidence requires human adjudication.
...
- D02 [OPEN]: Resolve the illustrative comparator in a versioned rule decision. Real clinical limits require qualified clinical ownership and separate validation. Do not mark both conflicting expected outcomes as passing the same rule version.
...
- **HC-005.T01:** Use a synthetic forbidden-class proposal and a patient/device contradiction. **Expected:** The former denies with reasons; the latter reviews and displays both sources, without automatic arbitration.
- **HC-005.T02:** Test just below, equal to and above a pedagogical ceiling. **Expected:** Equality acceptance is BLOCKED until D02 is resolved; no clinically authoritative threshold is inferred from the example.
...
Requirement IDs: HC-005. Acceptance case IDs: HC-005.T01, HC-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
