# OPR-BLD-001 - Versioned acceptance baseline
...
Freeze the release scope, source versions, supported runtimes and adapters before assigning completion status.
...
- **BLD-001.T01:** Record archive/commit and document hashes; rerun inventory after changing a source. **Expected:** Evidence refers to the exact candidate; a changed candidate invalidates inherited acceptance.
- **BLD-001.T02:** Compare advertised capabilities with acceptance rows and runtime registration. **Expected:** Missing, bounded, simulated and unsupported features remain explicit; a module name is not a pass.
...
Requirement IDs: BLD-001. Acceptance case IDs: BLD-001.T01, BLD-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
