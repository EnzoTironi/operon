# OPR-BLD-005 - Truthful test aggregation
...
Report required tests, skips, expected failures, diagnostics and blocked environments separately.
...
- **BLD-005.T01:** Remove a required package or database binding before the release check. **Expected:** The release is blocked; missing suites do not disappear from totals.
- **BLD-005.T02:** Run a probe that succeeds only when a security defect is reproduced. **Expected:** Its zero exit status is classified as reproduced defect, never a passing product test.
...
Requirement IDs: BLD-005. Acceptance case IDs: BLD-005.T01, BLD-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
