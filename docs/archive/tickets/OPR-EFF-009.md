# OPR-EFF-009 - Correct real driver integration
...
Run actual database and protocol integrations with real Effect; test doubles are allowed only at explicitly named boundaries.
...
- **EFF-009.T01:** Exercise SQL begin/write/fail/commit/reopen through the shipped Effect adapter, not raw SQL alone. **Expected:** The adapter passes the same behavioral contract and failure tests as the reference implementation.
- **EFF-009.T02:** Generate tools and SDK types from the actual installed Effect Schema definitions. **Expected:** Version-specific AST/projection assumptions are exercised and fail clearly when unsupported.
...
Requirement IDs: EFF-009. Acceptance case IDs: EFF-009.T01, EFF-009.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
