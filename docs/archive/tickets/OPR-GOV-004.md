# OPR-GOV-004 - Shadow comparison with governed cutover
...
Compare old/new rules and models without side effects; retain differences and the release decision.
...
- D06 [OPEN]: Define the release-specific policy. Under the literal no-differences rule, block until empty. A policy permitting approved intentional differences requires an explicit authorized amendment, not a silent weakening by an implementer.
...
- **GOV-004.T01:** Evaluate historical/current cases under two versions. **Expected:** Differences name inputs, verdicts and both versions; no shadow writeback.
- **GOV-004.T02:** Attempt activation with unreviewed shadow differences and with an approved intentional change. **Expected:** Follow the resolved cutover rule; unexplained differences cannot disappear.
...
Requirement IDs: GOV-004. Acceptance case IDs: GOV-004.T01, GOV-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
