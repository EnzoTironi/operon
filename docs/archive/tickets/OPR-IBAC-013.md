# OPR-IBAC-013 - Risk adds controls, not privileges
...
Contextual risk returns required controls or denial while preserving the actor autonomy ceiling and hard constraints.
...
- **IBAC-013-A:** Run permitted low/high-impact actions with proposal-only and bounded-autonomy actors. **Expected:** Required review varies with context, but no actor exceeds its authorized mode.
- **IBAC-013-B:** Set declaredBlastRadius=low or return dynamicTier=4 for a proposal-only caller. **Expected:** The actor remains proposal-only; uncertain impact cannot silently become low risk.
...
Requirement IDs: IBAC-013. Acceptance case IDs: IBAC-013-A, IBAC-013-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
