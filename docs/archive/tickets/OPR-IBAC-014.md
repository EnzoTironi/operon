# OPR-IBAC-014 - Cumulative impact and concurrency
...
The grant defines aggregate budgets and relationship scope; reservations prevent split-action and concurrent overspend.
...
- **IBAC-014-A:** Execute a sequence within the scoped budget and verify durable reservations and releases. **Expected:** Aggregate effects remain within the task/customer/time-window limit, including after restart.
- **IBAC-014-B:** Execute many individually small actions concurrently against different rows that share one remaining budget. **Expected:** The combined committed effects cannot exceed the bound; row revisions alone do not satisfy this test.
...
Requirement IDs: IBAC-014. Acceptance case IDs: IBAC-014-A, IBAC-014-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
