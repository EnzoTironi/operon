# OPR-SEN-011 - Scoped suspension and post-execution monitoring
...
A sentinel finding may request a scope-limited pause through a separate control service. It does not imply universal rollback or unilateral platform-wide shutdown.
...
- **SEN-011.T01:** Detect a high-severity anomaly, pause the affected grant/queue and reconcile commands already sent. **Expected:** Future work in the authorized scope stops, in-flight state is classified accurately and resume requires independent authority.
- **SEN-011.T02:** Forge a global shutdown request or race suspension with an already committed irreversible operation. **Expected:** The unauthorized scope is refused and the committed outcome is reconciled, not represented as undone.
...
Requirement IDs: SEN-011. Acceptance case IDs: SEN-011.T01, SEN-011.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
