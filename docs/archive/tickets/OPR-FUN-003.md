# OPR-FUN-003 - Materialization and invalidation
...
Materialized derived outputs preserve source and logic lineage and become invalid when required dependencies change.
...
- **FUN-003.T01:** Change an input without changing the materialized record manually. **Expected:** The output is invalidated/recomputed or visibly stale according to policy.
- **FUN-003.T02:** Release new logic and read the previous output. **Expected:** Its old value/input/logic lineage remains available for audit.
...
Requirement IDs: FUN-003. Acceptance case IDs: FUN-003.T01, FUN-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
