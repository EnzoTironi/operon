# OPR-CR-06 - Repair: Approval effects are claimed eagerly and can be rerun
...
Move checks and claim acquisition into the execution-time transaction; use idempotent operation identity and CAS consumption; recover abandoned claims.
...
- packages/runtime/src/inbox.ts:104-109
- packages/runtime/src/inbox.ts:197-243
...
- **CR-06.REGRESSION:** Real Effect construction without run, repeated run, fiber interruption, concurrent reviewers, retries and restart; approved operation produces only one authorized logical execution. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
...
Requirement IDs: APR-001. Acceptance case IDs: APR-001.T01, APR-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
