# OPR-VAL-004 - Persistent CLI round trips
...
The persistent CLI profile must share qualified durable objects, audit, proposals, OMS and operations across fresh processes; no split best-effort JSON authority store.
...
- **VAL-004.T01:** Submit an action then read its object and audit in separate processes **Expected:** Object and dossier agree after restart
- **VAL-004.T02:** Crash before/after commit and run two writers concurrently **Expected:** No orphan accepted state, erased concurrent write or fabricated recovery
...
Requirement IDs: VAL-004. Acceptance case IDs: VAL-004.T01, VAL-004.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
