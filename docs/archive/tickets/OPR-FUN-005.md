# OPR-FUN-005 - Six-part prediction contract
...
A prediction includes value, uncertainty, input snapshot, version chain, scope check and decision readiness.
...
- **FUN-005.T01:** Compute a model output and inspect all six groups. **Expected:** Every group is populated or explicitly unavailable; confidence is separate from readiness.
- **FUN-005.T02:** Return high confidence with missing required evidence. **Expected:** Readiness remains incomplete and the governing action does not bypass the evidence policy.
...
Requirement IDs: FUN-005. Acceptance case IDs: FUN-005.T01, FUN-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
