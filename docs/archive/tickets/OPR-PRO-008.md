# OPR-PRO-008 - No replay of simulated external outcomes
...
Real commands are generated from an approved production commitment with fresh environment-bound idempotency and delivery records; simulator receipts are evidence only.
...
- **PRO-008.T01:** Promote a plan whose scenario generated many simulated notifications; observe the independent real receiver. **Expected:** Only authorized production commands are sent; each logical effect is tracked and deduplicated where supported.
- **PRO-008.T02:** Replay a scenario outbox or report simulated acceptance as remote confirmation after a live timeout. **Expected:** The entry is rejected and live uncertainty remains unresolved until actual reconciliation.
...
Requirement IDs: PRO-008. Acceptance case IDs: PRO-008.T01, PRO-008.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
