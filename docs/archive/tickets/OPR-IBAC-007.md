# OPR-IBAC-007 - Output and recipient constraints
...
Writes, generated outputs, notifications and external recipients must satisfy the task grant and data-use restrictions.
...
- **IBAC-007-A:** Prepare a permitted aggregate report and send it to its approved recipient group. **Expected:** Disclosure and output markings match the grant; the result remains governed.
- **IBAC-007-B:** Place sensitive source text in a supposedly harmless message, URL, log, or external command recipient. **Expected:** Preparation or delivery blocks the unauthorized disclosure; write checks alone are not considered sufficient.
...
Requirement IDs: IBAC-007. Acceptance case IDs: IBAC-007-A, IBAC-007-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
