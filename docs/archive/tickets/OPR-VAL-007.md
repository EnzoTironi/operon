# OPR-VAL-007 - Execute historical CLI queries
...
Separate query execution from SQL explanation; historical query returns actual authorized rows and WorldView coordinates.
...
- **VAL-007.T01:** Create two versions and execute a past-time query in a new process **Expected:** Historical rows match an independent interval oracle, not merely printed SQL
- **VAL-007.T02:** Request explain and execute for a denied or unsupported query **Expected:** Modes are labeled; denied/unsupported semantics cannot return plausible results
...
Requirement IDs: VAL-007. Acceptance case IDs: VAL-007.T01, VAL-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
