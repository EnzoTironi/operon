# OPR-VAL-010 - Hermetic verification state
...
Each run owns isolated database/state/evidence paths; prior local files cannot make a clean-run test pass or contaminate another test.
...
- **VAL-010.T01:** Execute two full smoke runs in parallel and one after corrupt old state **Expected:** No cross-run proposals, branch names, records or evidence overwrite
- **VAL-010.T02:** Run clean fixture, modify data, then reread with an independent observer **Expected:** Fixture setup and business effects are distinct and demonstrable
...
Requirement IDs: VAL-010. Acceptance case IDs: VAL-010.T01, VAL-010.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
