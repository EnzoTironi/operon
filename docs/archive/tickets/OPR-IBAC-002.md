# OPR-IBAC-002 - Versioned purpose registry
...
Purposes are governed identifiers with immutable versions, ownership, permitted use and explicit restrictions; arbitrary strings are not authorization.
...
- **IBAC-002-A:** Publish a purpose through branch review and issue a grant referencing that exact version. **Expected:** The registry retains the version and reviewer; the grant resolves the published policy.
- **IBAC-002-B:** An agent creates emergency_override or edits a purpose while proposing work. **Expected:** Neither a new purpose nor broader permission becomes active through the action submission.
...
Requirement IDs: IBAC-002. Acceptance case IDs: IBAC-002-A, IBAC-002-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
