# OPR-VAL-001 - Fail-closed verifier outputs
...
The verifier must validate the expected structured result and business assertions for every step and reject malformed output or semantic failure even when the child exits zero.
...
- **VAL-001.T01:** Inject failed JSON into every CLI step **Expected:** Nonzero verifier status and no all-verified result
- **VAL-001.T02:** Inject a non-JSON zero-exit child; then a valid expected response **Expected:** Malformed output is rejected; a valid positive control passes
...
Requirement IDs: VAL-001. Acceptance case IDs: VAL-001.T01, VAL-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
