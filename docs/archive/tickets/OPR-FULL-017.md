...
Registrar intenção, decisão e efeitos locais de forma atômica.
...
- **FULL-ACC-017:** Given uma ação local com efeito externo chega ao compromisso; when a transação falha no último write **Expected:** não resta efeito local parcial nem mensagem de outbox órfã
...
Requirement IDs: FULL-017. Acceptance case IDs: FULL-ACC-017. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
