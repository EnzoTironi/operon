# OPR-IBAC-030 - Independent release evidence
...
Release claims are tied to exact builds, real dependencies, declared workloads and evidence types; source inspection and simulations remain labeled.
...
- **IBAC-030-A:** Run the complete candidate in an independent clean environment with held-out tasks, real Effect/adapters and declared workload targets. **Expected:** Report exact versions, commands, artifacts, denominators, latency and outcome metrics with reproducible evidence.
- **IBAC-030-B:** Remove a dependency, skip a safety test, change source after testing, or substitute a fake adapter in a purported live suite. **Expected:** The corresponding gate is BLOCKED/NOT_RUN/FAIL as appropriate; it cannot remain PASS or inflate coverage.
...
Requirement IDs: IBAC-030. Acceptance case IDs: IBAC-030-A, IBAC-030-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
