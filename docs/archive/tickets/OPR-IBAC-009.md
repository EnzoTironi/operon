# OPR-IBAC-009 - Decision-relative evidence completeness
...
The policy defines required evidence independently of the agent; missing, stale and conflicting evidence have explicit non-permitting semantics.
...
- **IBAC-009-A:** Evaluate a complete fresh evidence set at exact freshness boundaries with declared coverage. **Expected:** The verdict follows the versioned boundary rules and reports each required evidence item.
- **IBAC-009-B:** Omit an unfavorable observation, use an unrelated recent update to refresh a stale fact, or self-declare all questions answered. **Expected:** No self-report bypasses completeness or freshness; the prescribed UNKNOWN/REVIEW/DENY is returned without effects.
...
Requirement IDs: IBAC-009. Acceptance case IDs: IBAC-009-A, IBAC-009-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
