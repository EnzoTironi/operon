# OPR-CR-02 - Repair: Temporal corrections rewrite historical knowledge and diverge by path
...
Use immutable knowledge revisions and interval-splitting rules; define valid/knowledge coordinates; unify direct and action-batch paths; include scopes and deterministic revision ordering.
...
- packages/runtime/src/sql-store.ts:101-143
- packages/runtime/src/sql-store.ts:374-415
- packages/runtime/src/sql-store.ts:458-490
- packages/runtime/src/sql-store.ts:570-680
...
- **CR-02.REGRESSION:** Independent interval oracle across retroactive/future/bounded corrections, same-tick writes, links, deletes, current/as-of reads and every production adapter. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
