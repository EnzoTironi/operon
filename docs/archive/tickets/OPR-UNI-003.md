# OPR-UNI-003 - Adversarial compositions and independent oracles
...
Acceptance combines cross-boundary failures and validates against independent expected outcomes, not exclusively agent-authored tests or sentinel scores.
...
- **UNI-003.T01:** Run state-machine/property tests for cross-tenant scenario clone, injected evidence, concurrent budgets and stale approval. **Expected:** Independent invariants hold under declared seeds and schedules; each failed case is minimized and retained.
- **UNI-003.T02:** Deliberately disable authentication, scenario routing, effect checks and promotion filtering one at a time. **Expected:** The suite detects each mutation; self-edited local passing reports cannot satisfy acceptance.
...
Requirement IDs: UNI-003. Acceptance case IDs: UNI-003.T01, UNI-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
