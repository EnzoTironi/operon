# OPR-IBAC-018 - External delivery and grant lifecycle
...
Dispatch, uncertainty, expiry/revocation after commit and compensation follow a declared per-action external commitment contract.
...
- **IBAC-018-A:** Deliver an authorized persisted command with a stable identifier and record confirmed external state. **Expected:** Only the authorized payload/recipient is sent and the receipt reflects confirmed rather than merely queued status.
- **IBAC-018-B:** Time out after the external system accepts; then expire the grant and restart a competing worker. **Expected:** No blind unsafe duplicate; fencing, reconciliation and the documented post-commit expiry policy apply.
...
Requirement IDs: IBAC-018. Acceptance case IDs: IBAC-018-A, IBAC-018-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
