# OPR-IBAC-020 - Canonical audit and trust anchors
...
Hash exactly the persisted supported representation; state the trust basis and distinguish hash chains, Merkle proofs and signatures.
...
- **IBAC-020-A:** Serialize, persist, reload and verify intent-bearing records and an independently anchored checkpoint. **Expected:** Digests and anchor verification remain valid under the declared representation.
- **IBAC-020-B:** Mutate optional undefined values across JSON, rewrite local history and hashes, or fabricate a Merkle inclusion claim from a linear chain. **Expected:** Unsupported representation is rejected/normalized; tampering is detected where the stated independent anchor permits; unsupported proofs are not advertised.
...
Requirement IDs: IBAC-020. Acceptance case IDs: IBAC-020-A, IBAC-020-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
