# OPR-VAL-012 - Actual MCP lifecycle tests
...
Verification must initialize, negotiate, discover, call and close the actual pinned MCP transport with authorized identities; handlers alone do not prove protocol behavior.
...
- **VAL-012.T01:** Drive a real MCP client through initialize/list/call/close **Expected:** Results and identity propagation match the public protocol contract
- **VAL-012.T02:** Terminate stdin/cancel a request and inspect child process and stores **Expected:** Resources close and committed state survives without stdout corruption
...
Requirement IDs: VAL-012. Acceptance case IDs: VAL-012.T01, VAL-012.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
