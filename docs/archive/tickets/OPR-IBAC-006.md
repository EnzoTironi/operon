# OPR-IBAC-006 - Purpose-scoped reads and discovery
...
Task and purpose checks apply before tool discovery, object reads, traversal, search, aggregation and evidence exposure.
...
- **IBAC-006-A:** Run a task using only its permitted fields and related objects through every public read interface. **Expected:** The same permitted view is exposed consistently; caches include relevant authorization scope.
- **IBAC-006-B:** Fetch unrelated sensitive data through readiness, search counts, audit, inbox or shared cached results. **Expected:** No unauthorized data or existence signal beyond the explicitly approved disclosure policy is returned.
...
Requirement IDs: IBAC-006. Acceptance case IDs: IBAC-006-A, IBAC-006-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
