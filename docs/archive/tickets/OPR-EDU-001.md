# OPR-EDU-001 - Waiver domain, R1–R5 and version justice
...
Model students/programmes/courses/evidence/waivers, acyclic prerequisites and admittedUnder curriculum binding.
...
- D04 [OPEN]: Preserve all stated semantics, publish an unambiguous stable rule-ID crosswalk, and independently test admission-version binding. Do not infer that mislabeled clauses remove a table requirement.
...
- **EDU-001.T01:** Use a CAPSTONE request with excellent equivalent-course evidence and director willingness. **Expected:** The hard non-waivable rule denies and identifies its stable semantic rule ID.
- **EDU-001.T02:** Evaluate an identical request for cohorts bound to different rule versions. **Expected:** The admittedUnder binding selects the version, with both rule bases retained.
...
Requirement IDs: EDU-001. Acceptance case IDs: EDU-001.T01, EDU-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
