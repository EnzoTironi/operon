# OPR-FDE-001 - Actual natural-language engineering path
...
For the advertised supported scope, generate a plan and concrete pipeline/repository/ontology changes from natural language rather than requiring a prebuilt change set.
...
- **FDE-001.T01:** Give the real configured model/provider new requests involving source mapping, transformations and ontology maintenance. **Expected:** It produces inspectable changes that compile/run in a sandbox; a regex command parser is labeled as such, not a general AI FDE.
- **FDE-001.T02:** Run without model credentials or force malformed/provider-failure responses. **Expected:** Feature is explicitly unavailable/recoverable, not fulfilled by fabricated output.
...
Requirement IDs: FDE-001. Acceptance case IDs: FDE-001.T01, FDE-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
