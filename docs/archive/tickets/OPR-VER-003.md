# OPR-VER-003 - Type reliability proof obligation
...
State/prove the modeled no-dangling-reference property, resolving how intra-action creation and linking are represented.
...
- D07 [OPEN]: Define the selected action fragment and transition semantics: either exclude new-reference effects in that theorem or prove a correctly stated extension with well-scoped fresh identities. Do not claim the literal theorem covers an unmodeled case.

## Exact implementation locations

- packages/runtime/src/verification.ts
- proposed packages/formal/

## Steps

1. Resolve and freeze the relevant S19 contract; retain existing source vocabulary and open decisions.
...
- **VER-003.T01:** Execute modeled create/link/delete/update sequences including new intra-action references. **Expected:** Valid creation is modeled explicitly; dangling references produce a counterexample.
- **VER-003.T02:** Review machine-checkable proof or rigorous argument for the chosen fragment and its mapping to the runtime. **Expected:** Claim is scoped to proven semantics; unit tests are not substituted for the theorem.
...
Requirement IDs: VER-003. Acceptance case IDs: VER-003.T01, VER-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
