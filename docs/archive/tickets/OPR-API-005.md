# OPR-API-005 - Tool projection and filtered discovery
...
Expose only permitted tools/fields and reject unsupported schema projections.
...
- **API-005.T01:** Project actual Effect schemas for refined values, unions, arrays, nullability and object references. **Expected:** Supported semantics are preserved; unsupported refinements are declared, not silently weakened.
- **API-005.T02:** List tools/schema metadata under observer, proposer and builder identities. **Expected:** Discovery follows exposure policy; guessing hidden tool names still fails runtime authorization.
...
Requirement IDs: API-005. Acceptance case IDs: API-005.T01, API-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
