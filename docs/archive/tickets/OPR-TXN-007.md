# OPR-TXN-007 - Consistent adapter contracts
...
Memory/reference and production adapters share behavioral semantics, while durability claims require real persistence tests.
...
- **TXN-007.T01:** Run CRUD/link/history/action tests against every claimed adapter. **Expected:** Equivalent logical behavior; backend-specific unsupported features are explicit.
- **TXN-007.T02:** Exercise lock contention, transaction nesting, rollback failure and busy timeouts through the actual driver. **Expected:** Documented errors/retry policy; no leaked transaction or untracked partial state.
...
Requirement IDs: TXN-007. Acceptance case IDs: TXN-007.T01, TXN-007.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
