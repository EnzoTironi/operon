# OPR-IBAC-001 - Trusted task origin
...
Only authenticated principals or authorized workflow events may establish an authoritative mandate. Agent text and retrieved content remain proposals.
...
- **IBAC-001-A:** Create a task through an authenticated user session and resolve the persisted issuer, tenant, beneficiary and immutable request digest. **Expected:** The mandate identifies the verified issuer and original request; injected identity fields cannot change either.
- **IBAC-001-B:** Submit the same task using an unsigned token, another tenant identifier, or an instruction extracted from a document. **Expected:** No authoritative mandate or grant is created. The attempt has an appropriately protected failure record.
...
Requirement IDs: IBAC-001. Acceptance case IDs: IBAC-001-A, IBAC-001-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
