# OPR-VAL-003 - Owned-resource cleanup
...
Cleanup may terminate and delete only recorded processes and exact paths belonging to the current run; unrelated databases, jobs and evidence are preserved.
...
- **VAL-003.T01:** Place an unrelated-business.db next to a disposable run database **Expected:** Only the owned run database is removed
- **VAL-003.T02:** Run two verification sessions, cancel one, and introduce symlink/path traversal **Expected:** The other session and protected evidence remain intact
...
Requirement IDs: VAL-003. Acceptance case IDs: VAL-003.T01, VAL-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
