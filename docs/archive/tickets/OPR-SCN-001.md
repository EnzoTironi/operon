# OPR-SCN-001 - Trusted scenario identity and non-transferable grants
...
Every operation is bound by the server to tenant, ontology, branch, scenario and execution profile; scenario grants are audience-scoped and never valid on production resources.
...
- **SCN-001.T01:** Perform scenario mutations with a server-issued scenario grant across HTTP, MCP, SDK and jobs. **Expected:** All reads/writes resolve the intended scenario scope; no caller-selected mode changes authority.
- **SCN-001.T02:** Remove or forge scenarioId, replay the token at production, and invoke a nested action without context. **Expected:** The call fails closed; missing context never defaults to production.
...
Requirement IDs: SCN-001. Acceptance case IDs: SCN-001.T01, SCN-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
