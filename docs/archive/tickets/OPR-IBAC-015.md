# OPR-IBAC-015 - Exact independent approval
...
Approval is a durable record bound to verified actors, evidence, purpose, grant, parameters, effects, policy and handler versions.
...
- **IBAC-015-A:** Obtain the required independent quorum for an immutable prepared action and execute it once. **Expected:** The approved scope is consumed atomically and distinct requester/reviewer/executor identities remain visible.
- **IBAC-015-B:** Self-approve, duplicate a reviewer under an alias, change a target after approval or supply isApprovedProposal=true. **Expected:** The approval requirement is not satisfied and the mutated operation cannot execute.
...
Requirement IDs: IBAC-015. Acceptance case IDs: IBAC-015-A, IBAC-015-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
