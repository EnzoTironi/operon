...
Translate any advertised aviation/nursing/claims example into an explicit domain contract; historical vendor outcomes are not Operon acceptance targets.
...
- D09 [OPEN]: For each advertised optional domain package, obtain an explicit owner-approved workflow contract and test corpus. Otherwise retain these passages as context; do not invent rules or borrow the vendor impact statistics as Operon outcomes.

## Exact implementation locations

- packages/runtime/src/readiness.ts
- packages/runtime/src/sandbox.ts
- proposed packages/missions/

## Steps

1. Resolve and freeze the relevant S12 contract; retain existing source vocabulary and open decisions.
...
- **CTX-001.T01:** For each shipped optional example, inspect source objects, intended decisions, guarded actions and external integration claims. **Expected:** A domain owner defines missing detailed rules; narrative figures are not silently turned into synthetic success criteria.
- **CTX-001.T02:** Compare product documentation with actual tested scale and integrations. **Expected:** Airbus/SOMPO statistics and partnerships are not presented as outcomes or capabilities achieved by this product.
...
Requirement IDs: CTX-001. Acceptance case IDs: CTX-001.T01, CTX-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
