# OPR-CR-09 - Repair: Approval binds parameters but not immutable action implementation
...
Use immutable action releases and handler digests, reviewed edit plans and content-addressed policy/evidence references. Freeze/copy permitted value objects, not arbitrary functions.
...
- packages/runtime/src/inbox.ts:61-93
- packages/runtime/src/inbox.ts:169-193
...
- **CR-09.REGRESSION:** Replace action code, policy, relevant evidence or schema after approval; the old grant cannot silently authorize materially different execution. **Expected:** Exact historical counterexample fails safely while the corresponding legitimate operation succeeds; real dependencies and external observations required.
...
Requirement IDs: ACT-001. Acceptance case IDs: ACT-001.T01, ACT-001.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
