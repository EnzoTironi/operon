# OPR-IBAC-022 - Scoped break-glass process
...
Emergency authorization is independently issued, time-bound, narrowly scoped, auditable and limited to explicitly overrideable rules.
...
- **IBAC-022-A:** Activate an authorized incident-specific exception and satisfy its post-action review obligation. **Expected:** Only the declared exception scope changes and follow-up remains tracked.
- **IBAC-022-B:** Rename a routine purpose emergency_override or try to waive a non-waivable constraint. **Expected:** No privilege changes or prohibited action becomes allowed.
...
Requirement IDs: IBAC-022. Acceptance case IDs: IBAC-022-A, IBAC-022-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
