# OPR-IBAC-011 - Effect-free action preparation
...
Preparation uses a registered versioned handler and restricted capabilities; it stages a full footprint without committing effects.
...
- **IBAC-011-A:** Construct and then run a prepare operation while inspecting storage and network calls independently. **Expected:** Construction is lazy; preparation returns the read/write/command footprint but commits no business or external change.
- **IBAC-011-B:** Run a handler that attempts direct SQL, hidden network delivery or a write outside the staging interface. **Expected:** The capability boundary prevents the operation; a successful preview cannot conceal an immediate side effect.
...
Requirement IDs: IBAC-011. Acceptance case IDs: IBAC-011-A, IBAC-011-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
