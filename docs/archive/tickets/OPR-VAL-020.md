# OPR-VAL-020 - Verification resources and secret-safe artifacts
...
Verification must bound processes, wall time, outputs and model spend; preserve needed evidence while keeping secrets and private payloads out of broadly shared artifacts.
...
- **VAL-020.T01:** Hang a child, emit huge output, inject terminal escapes and sensitive tokens **Expected:** Owned run is bounded, diagnostics encoded, secrets withheld
- **VAL-020.T02:** Run a legitimate bounded task with sufficient observability **Expected:** Useful evidence remains verifiable without exposing credentials
...
Requirement IDs: VAL-020. Acceptance case IDs: VAL-020.T01, VAL-020.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
