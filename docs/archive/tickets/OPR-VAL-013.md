# OPR-VAL-013 - Assertions and input sensitivity
...
**Depends on:** OPR-VAL-001, OPR-VAL-002
...
Smoke tests require explicit domain-independent oracles, positive and negative assertions, and sensitivity to changed input—not only exit codes.
...
- **VAL-013.T01:** Change model input from 10 to 12 and compare the expected value **Expected:** A stale --input/--inputs mismatch is detected
- **VAL-013.T02:** Mutate readiness status, sum result or proposal target while retaining exit zero **Expected:** Tests fail at the semantic assertion, not after unrelated errors
...
Requirement IDs: VAL-013. Acceptance case IDs: VAL-013.T01, VAL-013.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
