# OPR-TXN-002 - Crash and interruption recovery
...
Recover consistent committed state after process termination at defined transaction barriers.
...
- **TXN-002.T01:** Use a second process to terminate the writer after a staged mutation before commit, then reopen the database. **Expected:** No partial business/audit/approval/outbox unit survives.
- **TXN-002.T02:** Terminate after commit but before success reaches the client. **Expected:** Status/idempotency lookup recovers the original committed result with no duplicate action.
...
Requirement IDs: TXN-002. Acceptance case IDs: TXN-002.T01, TXN-002.T02, BAI-037, BAI-038, BAI-039, BAI-040, BAI-041, BAI-042. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
