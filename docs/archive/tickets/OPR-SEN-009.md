# OPR-SEN-009 - Review policy, model and prompt release governance
...
Reviewer prompts, policies, tools, model selection and evaluation corpus versions are governed artifacts; agent edits cannot silently replace the live reviewer.
...
- **SEN-009.T01:** Release a candidate review configuration through shadow comparison, independent review and rollback. **Expected:** The active version and evaluation evidence are linked and every subsequent finding identifies its configuration.
- **SEN-009.T02:** A builder edits reviewer instructions in its scenario and tries to replace the active deployment or reuse old positive reviews. **Expected:** Changes remain candidate artifacts; activation and review reuse follow independent version checks.
...
Requirement IDs: SEN-009. Acceptance case IDs: SEN-009.T01, SEN-009.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
