# OPR-UNI-005 - Release evidence tied to candidate and profile
...
Each release includes exact code/lockfile/config/model/policy/environment/fixture identities, all applicable results, exclusions and unresolved decisions, with independent reviewer reproduction.
...
- **UNI-005.T01:** Assemble the candidate evidence manifest and independently reproduce the advertised profile on its documented target. **Expected:** Required FAIL/BLOCKED/NOT_RUN cases and unresolved dependent decisions prevent that claim; all simulated/provider boundaries are visible.
- **UNI-005.T02:** Reuse results after code/model/security-boundary changes or omit failed tests from the report. **Expected:** Digest and completeness checks invalidate inherited evidence; no universal safety or completeness percentage is inferred.
...
Requirement IDs: UNI-005. Acceptance case IDs: UNI-005.T01, UNI-005.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
