# V1-05 - Atomic execution, delivery and reconciliation
...
- None. Conforms strictly to S08 normative contract and Effect v4 discipline.

## Exact paths

- `packages/schema/src/actions.ts`
- `packages/runtime/src/transactions/atomic-commit-service.ts`
- `packages/runtime/src/transactions/atomic-commit-service.test.ts`
- `packages/runtime/src/actions.test.ts`

## Environment

- `OPERON_DATABASE_URL`
- `OPERON_OUTBOX_POLL_MS`
- `OPERON_RECONCILE_TIMEOUT_MS`
- `OPERON_EVIDENCE_DIR`
...
```ts
execute(approval, key): Promise<OperationReceipt>;
reconcile(operationId): Promise<OperationReceipt>;
```

## Steps

1. Freeze the touched schemas and typed errors (`OperationReceiptSchema`, `OutboxItemSchema`, `IdempotencyConflictError`, `CommitConcurrencyError`).
...
- [x] commit plus outbox enqueue is atomic.
- [x] same idempotency key plus changed input conflicts.
- [x] crash at every fault point yields one effect or explicit UNKNOWN then reconciled truth.
