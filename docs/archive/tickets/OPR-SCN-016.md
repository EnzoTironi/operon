# OPR-SCN-016 - Candidate policy experiments are not policy activation
...
A builder can explore schema/policy/code changes in an isolated candidate branch, separate from a data scenario; changes to definitions require their own release route and tests.
...
- **SCN-016.T01:** Test revised business criteria against the same pinned inputs and compare with the active policy. **Expected:** Differences are labeled by candidate version and remain isolated from production authority.
- **SCN-016.T02:** Merge a data scenario carrying permissive copied policy rows and attempt to activate them. **Expected:** The data-promotion route rejects control artifacts and directs definition changes to reviewed release governance.
...
Requirement IDs: SCN-016. Acceptance case IDs: SCN-016.T01, SCN-016.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
