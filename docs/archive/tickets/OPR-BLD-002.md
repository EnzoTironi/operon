# OPR-BLD-002 - Real dependency installation
...
Use the real selected Effect release and a reviewed lockfile, with a recorded Node/package-manager/toolchain matrix.
...
- **BLD-002.T01:** Install from an empty workspace using the frozen lockfile; resolve/import every runtime dependency. **Expected:** Installation and imports succeed without undeclared global tools; unresolved dependencies are BLOCKED, not skipped.
- **BLD-002.T02:** Change a dependency manifest without updating the lockfile and run the frozen installation. **Expected:** The build refuses incompatible lockfile state; records package versions and integrity information.
...
Requirement IDs: BLD-002. Acceptance case IDs: BLD-002.T01, BLD-002.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
