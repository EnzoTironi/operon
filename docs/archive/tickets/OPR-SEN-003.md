# OPR-SEN-003 - Structured verdict and validated evidence references
...
Review outputs use a versioned schema with bundle digest, findings, evidence references, recommended controls, uncertainty, model identity and policy version. Model-provided references are resolved independently.
...
- **SEN-003.T01:** Obtain a live review containing supported structured findings and resolve each reference/version/span. **Expected:** Only resolvable findings are admitted; uncertainty and unsupported claims remain explicit.
- **SEN-003.T02:** Return malformed JSON, nonexistent evidence, truncated required findings or a fabricated approval identifier. **Expected:** No positive review condition is satisfied; the request is held or escalated under the configured policy.
...
Requirement IDs: SEN-003. Acceptance case IDs: SEN-003.T01, SEN-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
