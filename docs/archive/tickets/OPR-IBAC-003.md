# OPR-IBAC-003 - Explicit purpose compatibility
...
Purpose hierarchies, data-use constraints and compatible transformations must have declared semantics; name prefixes imply no authority.
...
- **IBAC-003-A:** Authorize a declared compatible use across a permitted purpose relationship and check all data restrictions. **Expected:** The decision identifies the compatibility rule and retains inherited restrictions.
- **IBAC-003-B:** Use a child purpose whose spelling resembles an allowed parent, or reuse treatment/research-style restricted data for an incompatible purpose. **Expected:** The operation is blocked unless an explicit reviewed policy permits that compatibility; string similarity never grants it.
...
Requirement IDs: IBAC-003. Acceptance case IDs: IBAC-003-A, IBAC-003-B. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
