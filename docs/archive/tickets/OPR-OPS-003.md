# OPR-OPS-003 - Backups, restoration and retention
...
Define recovery/retention targets and test consistent backup/restore of all decision-critical state and artifacts.
...
- **OPS-003.T01:** Take a supported consistent backup under concurrent activity and restore to a clean environment. **Expected:** Objects, versions, approvals, audit and pending commitments agree; restored replay works.
- **OPS-003.T02:** Remove a required historical rule/model/evidence asset from a recovery set. **Expected:** Restore validation detects the gap instead of silently claiming complete replay.
...
Requirement IDs: OPS-003. Acceptance case IDs: OPS-003.T01, OPS-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
