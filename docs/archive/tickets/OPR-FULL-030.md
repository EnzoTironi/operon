# OPR-FULL-030 - Verificação incremental
...
A avaliação incremental deve equivaler à avaliação completa no fragmento suportado, ou recuar à avaliação completa.
...
- **FULL-ACC-030:** Given uma mudança invalida dependências transitivas de uma regra; when as avaliações incremental e completa são comparadas **Expected:** os veredictos coincidem ou o modo incremental é bloqueado com contraexemplo
...
Requirement IDs: FULL-030. Acceptance case IDs: FULL-ACC-030. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
