# OPR-EDU-003 - Missingness and named review authority
...
Missing evidence routes to review/supplementation; work-experience evidence requires director review, not adviser discretion.
...
- **EDU-003.T01:** Use a request with a missing evidence status and no other hard denial. **Expected:** REVIEW with an evidence-missing reason, not an unexplained terminal denial.
- **EDU-003.T02:** Student submits, adviser verifies, director reviews work experience and registry supervises. **Expected:** Only the declared roles perform each transition; adviser cannot approve by precheck alone.
...
Requirement IDs: EDU-003. Acceptance case IDs: EDU-003.T01, EDU-003.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
