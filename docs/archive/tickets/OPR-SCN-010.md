# OPR-SCN-010 - Bounded compute, fan-out and retention cost
...
Per-task/scenario/tenant budgets bound elapsed time, CPU, memory, disk, model spend, child agents and queued work; nested workloads consume a shared budget.
...
- **SCN-010.T01:** Run parallel agents near each configured limit using independently observed counters. **Expected:** Consumption is reserved consistently and the declared envelope is never expanded by child tasks.
- **SCN-010.T02:** Trigger an infinite loop, fork storm, huge query and simultaneous model requests. **Expected:** Work stops or throttles at the policy boundary with preserved audit evidence; the agent cannot raise its own limit.
...
Requirement IDs: SCN-010. Acceptance case IDs: SCN-010.T01, SCN-010.T02. These identifiers preserve audit lineage; no external file is required to execute or judge this ticket.
