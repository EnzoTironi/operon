...
import { OAuthTestServer, serveTestHttpServerLayer } from "@operon/gateway/sdk/testing";
import {
  AuthTemplateSlug,
  ConnectionName,
  IntegrationSlug,
...
} from "@operon/gateway/sdk";
import type { OpenApiPluginExtension, OpenApiSpecConfig } from "@operon/gateway/source-openapi/plugin";
...
export interface MutableOpenApiSpecTestServerShape extends OpenApiTestServerShape {
  readonly specUrl: string;
  readonly setApi: (api: HttpApi.Top) => Effect.Effect<void, OpenApiTestServerSpecError>;
...
export const makeOpenApiTestSpecJson = (
  ...
  const annotated = (api as HttpApi.Top).annotateMerge(annotations);
...
> =>
  Effect.gen(function* () {
    ...
    const ApiLive = HttpApiBuilder.layer(options.api as HttpApi.Top).pipe(
...
export class OpenApiHttpApiTestServer extends Context.Service<
  ...
  OpenApiHttpApiTestServerShape
>()("@operon/gateway/source-openapi/testing/OpenApiHttpApiTestServer") {
...
export const serveMutableOpenApiSpecTestServer = (options: {
  readonly initialApi: HttpApi.Top;
  readonly specPath?: `/${string}`;
  readonly transformSpec?: (spec: Record<string, unknown>) => Record<string, unknown>;
}): Effect.Effect<
...
> =>
  Effect.gen(function* () {
    ...
    const renderSpec = (api: HttpApi.Top) =>
...
export class OpenApiEchoTestServer extends Context.Service<
  ...
  OpenApiEchoTestServerShape
>()("@operon/gateway/source-openapi/testing/OpenApiEchoTestServer") {
