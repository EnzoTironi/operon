// ---------------------------------------------------------------------------
// @operon/gateway/sdk — public surface (v2)
// ---------------------------------------------------------------------------

// Re-export the Effect/Schema/HttpApi primitives plugin authors need so a
// plugin can be written importing only from `@operon/gateway/sdk`.
...
// FumaDB integration.
export { fumadb } from "@operon/gateway/store";
export type { FumaDB } from "@operon/gateway/store";
export type { AbstractQuery, Condition, ConditionBuilder } from "@operon/gateway/store/query";
export { column, idColumn, schema as fumaSchema, table } from "@operon/gateway/store/schema";
export type {
  AnyColumn,
  AnySchema,
  AnyTable,
...
} from "@operon/gateway/store/schema";
...
// builder is host-internal and reachable via `@operon/gateway/sdk/host-internal`.
