...
import { detectInsufficientScope } from "@operon/gateway/sdk";
...
const headersFrom = (headers: unknown): Headers => {
  if (headers == null) return new Headers();
  if (headers instanceof Headers) return new Headers(headers);
  if (Array.isArray(headers)) return new Headers(headers as Array<[string, string]>);
  if (typeof headers === "object") return new Headers(headers as Record<string, string>);
  return new Headers();
};
...
class McpHttpTransportError extends Schema.TaggedError<McpHttpTransportError>()(
...
const applyBody = async (
  ...
  throw new TypeError("Unsupported MCP fetch body");
};
