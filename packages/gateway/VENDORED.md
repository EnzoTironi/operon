...
- Operon membrane in `src/`: writes are `WriteCandidate`, never `invoke`. Reads become a quarantine envelope for `AccountableIngestionService.ingestRawSource`. Gmail/IMAP connectors are read-only with `SecretRef` stubs.
