...
function parseJsonArg<T>(argValue?: string): T | undefined {
  if (!argValue) return undefined;
  if (argValue.startsWith("{") || argValue.startsWith("[")) {
    try {
      return JSON.parse(argValue) as T;
    } catch {
      return undefined;
    }
  }
    ...
    try {
      switch (sub) {
        case "evaluate": {
          ...
          const profileRaw =
            profileIndex === -1 ? "local" : (args[profileIndex + 1] ?? "local");
          const profile =
          ...
          const casesRaw = casesIndex === -1 ? undefined : args[casesIndex + 1];
          const testCases = parseJsonArg<readonly F1TestCase[]>(casesRaw) ?? [
            {
              assertions: 5,
              executionTimeMs: 45,
        ...
        case "mirror": {
          ...
          const consentScope = parseJsonArg<ConsentScope>(consentRaw) ?? {
          ...
          const corrections = parseJsonArg<readonly TraceableCorrection[]>(
            correctionsRaw
          ) ?? [
            {
          ...
          if (exit._tag === "Failure") {
            ...
            if (isJson) {
              ...
              console.error(`Assurance F2 Mirror Evaluation Failed: ${errStr}`);
            }
            return 1;
          }
        ...
        case "scan": {
          ...
          if (isJson) {
            ...
            for (const v of result.violations) {
              console.log(`  - [${v.classification}] ${v.path}: ${v.details}`);
            }
          }
