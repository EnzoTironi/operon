...
export async function runOms(args: string[]): Promise<number> {
  const group = args[0];
  const action = args[1];
  ...
  try {
    if (group === "branch" && action === "create") {
      ...
      const branch = await Effect.runPromise(
        ctx.oms.createBranch(branchName, author)
      );
    ...
    if (group === "proposal" && action === "review") {
      ...
      const proposal = await Effect.runPromise(
        ctx.oms.reviewProposal(proposalId, {
          comments,
          reviewedAt: Date.now(),
          reviewer,
          verdict,
    ...
    if (group === "proposal" && action === "merge") {
      ...
      const proposal = await Effect.runPromise(
        ctx.oms.mergeProposal(proposalId, author, {
