...
export async function runAction(args: string[]): Promise<number> {
  const sub = args[0];
    ...
    if (sub === "submit") {
      ...
      if (useStdin) {
        try {
          ...
          rawParameters = JSON.parse(stdinBuffer.trim() || "{}");
        } catch (error: unknown) {
      ...
      const subjectType = (typeIndex === -1 ? "agent" : args[typeIndex + 1]) as
        | "user"
        | "agent";
      ...
      const result = await Effect.runPromise(
        executeWritePipeline(submission, ctx.objectStore, ctx.auditStore)
      );
