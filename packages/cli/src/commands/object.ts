...
export async function runObject(args: string[]): Promise<number> {
  const sub = args[0];
  ...
  try {
    if (sub === "get") {
      ...
      const obj = await Effect.runPromise(
        ctx.objectStore.getObject(typeId as ObjectTypeId, id)
      );
      if (!obj) {
    ...
    if (sub === "put") {
      ...
      await Effect.runPromise(ctx.objectStore.putObject(instance));
