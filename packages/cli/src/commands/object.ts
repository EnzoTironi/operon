    ...
    (ctx) =>
      Effect.gen(function* () {
        if (sub === "get") {
          ...
          if (!obj) {
            console.error(
              `Error: Object '${id}' of type '${typeId}' not found.`
            );
            return 1;
          }
          if (isJson) {
        ...
        if (sub === "put") {
          ...
          const parsedExit = yield* Effect.try({
            try: () =>
              JSON.parse(args[propsIndex + 1]) as Record<string, unknown>,
            catch: (err) => err,
          }).pipe(Effect.exit);
          ...
          const properties = parsedExit.value;

          const version =
            versionIndex === -1
              ? 1
              : Math.trunc(Number(args[versionIndex + 1]));
          ...
          if (putExit._tag === "Failure") {
            console.error("Failed to put object:", putExit.cause);
            return 1;
          }
